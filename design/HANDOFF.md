...
التنفيذ الحي: `src/` + `base44/`. المعادلات على الخادم (`opsDerivations` وأشباهها)، ثم المرآة في الواجهة.
...
```
قسم الحضور (Attendance) — بعد دمج Ops في القاعدة المحلية.
مسار الوزارة/HCM: حضور → ورديات/إجازة → حزمة امتثال MHRSD.
```
...
| بطاقة المهمة: خطوات · مرفقات · تعليقات · تسجيل · شريط اعتماد | **مكتمل** (دُمج في `handoff/server-first-ops`) |
| اختبارات الصيغة + البوابة `scripts/test-ops-derivations.mjs` | **مكتمل** |

### الحضور والانصراف (Attendance) — حالة القسم

| بند | حالة |
| --- | --- |
| اشتقاقات (`attendanceDerivations`: سماح 10 · وردية 8س · حالات ثابتة) | **مكتمل** |
| بوابة خارج النطاق (قبول بمبرر / رفض) على الخادم | **مكتمل** |
| تسوية غياب ≤45 يوم بمستند — القيد الأصلي لا يُمحى | **مكتمل** |
| اعتماد/رفض إضافي — الساعات تبقى ظاهرة | **مكتمل** |
| شرائح تصفية بـ `ATT_STATUS` + حالة فارغة صريحة | **مكتمل** |
...
### تطبيق zip «قدرات التصميم المتاحة (1)» — 2026-08-12

| بند | حالة |
| --- | --- |
| تحديث مرجع `.tmp-design-caps/design_handoff_nirovera/` من الـ zip | **مكتمل** |
| شريط تنقل Platform بأربع مجموعات (يومي · قوى عاملة · موارد وامتثال · إدارة) | **مكتمل** |
| Landing: نص البطل + مقارنة ERP + أسعار ريال/موظف من التصميم | **مكتمل** |
| نسخ HTML/`support.js` | **مرفوض** (خادم/React فقط) |

انظر أيضًا: `.tmp-design-caps/design_handoff_nirovera/ZIP_APPLY_2026-08-12b.md`
...
| **Platform UI / أقسام داخلية (IA)** | **مكتمل على الفرع** (تنقل + لوحات أدوار + صقل أقسام — انظر أدناه) |
...
### إكمال واجهة الأقسام الداخلية (جولة 2) — 2026-08-12

طلب المستخدم: «اكمل التحديث كاملا».

**ما طُبّق في هذه الجولة (بدون نسخ HTML/`support.js`):**

| سطح | حالة | ملاحظات |
| --- | --- | --- |
| لوحة الموظف | **مكتمل** | `EmployeeDashboard` بلغة مركز القيادة (جاهزية يومي · مهامي · روابط إثبات) عبر `operations.list` |
| لوحة مدير المحطة / PGM | **مكتمل** | `StationManagerDashboard` يعيد استخدام `HandoffCommandBoard` بنطاق المحطة |
| Operations `/app/tasks` | **مكتمل** | تبويب قائمة/خطة · جدول أعمدة التصميم · نموذج إنشاء قابل للطي · العدّادات من الخادم |
| Attendance | **مكتمل** | رأس هادئ · تبويبات مقطّعة · شريط `attStats` (نسبة · تأخير · خارج النطاق · ساعات) |
| HSE `/app/safety` | **مكتمل** | رأس هادئ · `HseRatesPanel` بأهداف TRIR/LTIFR/DART + أيام بلا حادث |
| Inventory | **مكتمل** | WIP مخزون مُدرَج + بانر تنبيه أحمر + جدول هادئ في `StockAlertBoard` |
| Payroll / Expenses | **مكتمل** | رؤوس هادئة؛ `PayrollRunBoard` / `ExpenseBudgetBoard` يبقيان المصدر التشغيلي |

**متبقٍ خارج نطاق الواجهة الداخلية (ليس مانعًا لقول «التحديث مكتمل» داخل `/app`):**
- نشر دوال Base44 على بيئة الإنتاج إن لزم (`base44 deploy`) — يحتاج اعتمادات المستخدم
- دمج الفرع `handoff/server-first-ops` → `main`
- صقل بكسل إضافي اختياري (هرم حوادث HSE / PTW / CAPA كشاشات فرعية) — الخادم والمؤشرات موجودة؛ ليست حاجزًا للـ IA

**تحقق سريع:** `/preview` → دخول →  
`/app` (موظف / مدير محطة / تنفيذي) · `/app/tasks` · `/app/attendance` · `/app/safety` · `/app/inventory` · `/app/payroll` · `/app/expenses` · `/app/shifts` · `/app/leave` · `/app/org` · `/app/settings` · `/app/reports`.
