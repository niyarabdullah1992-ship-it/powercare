/** Assistant (المساعد الذكي) — scoped Q&A board with named gates.
 *  Design: NiroVera Platform.dc.html (isAssistant / ANSWERS / evidence chips).
 *  Answers for design prompts are derived from tenant facts — not a second LLM stack.
 *  Free-form chat stays on the existing client LLM path after server authorize + scope.
 */
...
export type AssistantActor = {
  ...
  /** Station ids the actor may see; empty + non-allStations = none. */
  stationIds?: string[] | null;
  /** When true, actor may see all stations in the company. */
  allStations?: boolean;
};
...
export type AssistantFacts = {
  ...
  budgetPctByStation?: Record<string, number>;
  companyOtHoursWeek?: number;
  stations: StationFact[];
  hazards: HazardFact[];
  assets: AssetDowntimeFact[];
  blockedTasks: BlockedTaskFact[];
};
...
export type PromptCatalogEntry = {
  ...
  textAr: string;
  textEn: string;
  /** Roles that may run this prompt; empty = any authenticated tenant user with assistant access. */
  roles?: string[];
  /** Topics / nav sections required (navVisibility-style). */
  sections?: string[];
};
...
export const SPOTLIGHT_PROMPT: PromptCatalogEntry = {
...
export function catalogById(id?: string | null): PromptCatalogEntry | null {
  if (!id) return null;
  if (id === SPOTLIGHT_PROMPT.id) return SPOTLIGHT_PROMPT;
  return PROMPT_CATALOG.find((p) => p.id === id) || null;
}

export function matchPromptByText(text?: string | null): PromptCatalogEntry | null {
...
export function actorMayAsk(
  actor: AssistantActor | null | undefined,
...
export function planAllowsAssistant(plan?: PlanLike | null): boolean {
  const sections = plan?.enabledSections || [];
  const features = plan?.enabledFeatures || [];
  // Missing plan config → allow (session already authenticated); FEATURE_DISABLED when explicitly off.
  if (!plan) return true;
  ...
  // Explicit empty features with sections present that include assistant still needs ai.
...
export function scopeBlockedTasks(
  tasks: BlockedTaskFact[],
  stationIds: Set<string>,
  companyId: string,
): BlockedTaskFact[] {
  return (tasks || []).filter(
...
export function checkAskGate(input: {
  ...
  /** True when the request channel is careers/public — always deny employee data. */
  careersChannel?: boolean;
  /** Requested companyId vs session companyId mismatch. */
  crossTenant?: boolean;
  /** navVisibility-style: sections the actor may open. */
  allowedSections?: string[] | null;
}): GateFail | { ok: true; entry: PromptCatalogEntry | null; question: string } {
  ...
  if (entry) {
    ...
    // When allowedSections provided, require assistant + at least one topic section overlap (or assistant alone).
...
function stationName(st: StationFact, lang: "ar" | "en") {
...
export function derivePromptAnswer(
  ...
  scopedStations: StationFact[],
  scopedHazards: HazardFact[],
  scopedAssets: AssetDowntimeFact[],
  scopedTasks: BlockedTaskFact[],
): DerivedAnswer | null {
  const byId = (id: string) => scopedStations.find((s) => s.id === id);
  ...
  if (promptId === "board_report") {
    ...
    const tasks = facts.taskCompletionPct
      ?? Math.round(
    ...
    return {
      ...
      answerAr:
        `جهّزت المسودة: مؤشر الجاهزية ${readiness} (${delta >= 0 ? "+" : ""}${delta})، إنجاز المهام ${tasks}%، ${days} يومًا بلا حادث يفقد وقت عمل${hotSt ? `، وتكلفة تشغيل ضمن الميزانية عدا ${stationName(hotSt, "ar")} عند ${hotBudget![1]}%` : ""}. تحتاج مراجعتك قبل الإرسال.`,
      answerEn:
        `Draft ready: readiness ${readiness} (${delta >= 0 ? "+" : ""}${delta}), task completion ${tasks}%, ${days} days without a lost-time incident${hotSt ? `, and operating cost within budget except ${stationName(hotSt, "en")} at ${hotBudget![1]}%` : ""}. It needs your review before sending.`,
      evidence: [
        { sourceAr: "تنفيذي", sourceEn: "EXEC", value: String(readiness), labelAr: "الجاهزية", labelEn: "Readiness" },
        { sourceAr: "العمليات", sourceEn: "OPS", value: `${tasks}%`, labelAr: "الإنجاز", labelEn: "Completion" },
...
export function buildAssistantBoard(input: {
  ...
  prompts: Array<PromptCatalogEntry & { allowed: boolean }>;
  answer: DerivedAnswer | null;
  stationCount: number;
} {
...
/** Demo facts aligned with Platform.dc.html assistant ANSWERS / evidence. */
export function demoAssistantFacts(companyId: string): AssistantFacts {
  return {
    companyId,
    readiness: 82,
    readinessDelta: 6,
