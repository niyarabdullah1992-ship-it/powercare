...
Deno.serve(async (req) => {
  try {
    ...
    if (body.action === "submit") {
      const beforeTaxAmount = Number(body.beforeTaxAmount); const taxAmount = Number(body.taxAmount); const afterTaxAmount = Number(body.afterTaxAmount); const itemCount = body.itemCount == null || body.itemCount === "" ? null : Number(body.itemCount); const amount = afterTaxAmount; const canPickStations = isManager || isFinance;
      ...
      if (!stationIds.length || !expenseTypes.includes(body.expenseType) || (body.expenseType === "other" && !customExpenseType) || beforeTaxAmount < 0 || taxAmount < 0 || amount <= 0 || !totalsMatch || (itemCount != null && (!Number.isInteger(itemCount) || itemCount < 1)) || !body.expenseDate || !body.receiptUrl) return Response.json({ error: "Invalid expense data" }, { status: 400 });
      await base44.asServiceRole.entities.ExpenseClaim.create({ companyId: auth.companyId, requesterId: auth.userId || "owner", requesterName: auth.name, stationId: stationIds[0], stationIds, stationScope, expenseType: body.expenseType, customExpenseType, beforeTaxAmount, taxAmount, afterTaxAmount, itemCount, amount, totalAmount: amount * stationIds.length, currency: "SAR", expenseDate: body.expenseDate, description: String(body.description || ""), receiptUrl: String(body.receiptUrl), status: "submitted", managerReviewedBy: null, managerReviewedAt: null, financeReviewedBy: null, financeReviewedAt: null });
