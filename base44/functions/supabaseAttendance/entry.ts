...
Deno.serve(async (req) => {
  try {
    ...
    if (action === "checkIn") {
      ...
      const now = new Date();
      const parts = (settings.work_start_time || "08:00").split(":").map(Number);
      const startMinutes = parts[0] * 60 + (parts[1] || 0);
      const nowMinutes = now.getHours() * 60 + now.getMinutes();
      const status = nowMinutes > startMinutes + (settings.late_threshold_minutes || 0) ? "late" : "present";
      const payload = {
        company_id: companyId,
      ...
      if (!res.ok) {
        return Response.json({ error: saved?.message || "Failed to check in — run: CREATE TABLE IF NOT EXISTS attendance (id uuid primary key default gen_random_uuid(), company_id text, employee_id text, employee_name text, station_id text, date text, check_in_at timestamptz, check_out_at timestamptz, status text, work_hours numeric, check_in_lat numeric, check_in_lng numeric, created_at timestamptz default now());" }, { status: 400 });
      }
    ...
    if (action === "checkOut") {
      ...
      const patchRes = await fetch(`${SUPABASE_URL}/rest/v1/attendance?id=eq.${encodeURIComponent(row.id)}`, {
        method: "PATCH",
        headers: { ...headers, Prefer: "return=representation" },
        body: JSON.stringify({ check_out_at: now.toISOString(), work_hours: workHours }),
      });
