...
function weeklySummaryHtml(companyName, stats, language = 'en') {
  ...
  return `<!DOCTYPE html><html lang="${language}" dir="${c.dir}"><body style="margin:0;padding:0;background:${BRAND_BG};font-family:Arial,Tahoma,sans-serif;direction:${c.dir};">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:${BRAND_BG};padding:34px 12px;"><tr><td align="center">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:600px;background:#ffffff;border:1px solid ${BRAND_BORDER};border-radius:12px;overflow:hidden;box-shadow:0 12px 36px rgba(20,40,75,.10);">
      <tr><td style="height:4px;background:${BRAND_GREEN};font-size:0;line-height:0;">&nbsp;</td></tr>
      <tr><td style="padding:26px 28px 24px;text-align:center;background:${BRAND_NAVY};"><img src="${POWERCARE_LOGO_URL}" width="68" height="68" alt="${BRAND_NAME}" style="display:block;margin:0 auto 11px;padding:7px;border-radius:8px;background:#ffffff;" /><div style="font-size:23px;font-weight:700;letter-spacing:1.4px;color:#ffffff;">${BRAND_NAME}</div><div style="width:38px;height:2px;margin:10px auto;background:${BRAND_GREEN};font-size:0;line-height:0;">&nbsp;</div><div style="font-size:12px;font-weight:600;letter-spacing:.35px;color:#A9D9C2;">${c.header}</div></td></tr>
      <tr><td style="padding:28px 28px 14px;text-align:${align};"><h1 style="margin:0 0 9px;font-size:22px;color:${BRAND_NAVY};">${c.hello}</h1><p style="margin:0 0 22px;font-size:14px;line-height:1.8;color:${BRAND_MUTED};">${c.intro} <strong style="color:${BRAND_NAVY};">${escapeHtml(companyName)}</strong> ${c.period}</p>
        ...
        <h2 style="margin:25px 0 10px;padding-${c.dir === 'rtl' ? 'right' : 'left'}:10px;border-${c.dir === 'rtl' ? 'right' : 'left'}:3px solid ${BRAND_GREEN};font-size:15px;color:${BRAND_NAVY};">${c.tasks}</h2><table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border:1px solid ${BRAND_BORDER};border-radius:10px;background:#ffffff;overflow:hidden;">${detailRow(c.newTasks, stats.newThisWeek)}${detailRow(c.active, stats.active)}${detailRow(c.review, stats.pendingReview)}${detailRow(c.overdue, stats.overdue, true)}</table>
        <h2 style="margin:23px 0 10px;padding-${c.dir === 'rtl' ? 'right' : 'left'}:10px;border-${c.dir === 'rtl' ? 'right' : 'left'}:3px solid ${BRAND_GREEN};font-size:15px;color:${BRAND_NAVY};">${c.reports}</h2><table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border:1px solid ${BRAND_BORDER};border-radius:10px;background:#ffffff;overflow:hidden;">${detailRow(c.dailyReports, stats.reportsThisWeek)}${detailRow(c.newNotices, stats.newComplaintsThisWeek)}${detailRow(c.openNotices, stats.openComplaints, true)}</table>
        <p style="margin:23px 0 10px;padding:15px 18px;border:1px solid ${BRAND_GREEN};border-radius:10px;background:${BRAND_NAVY};font-size:13px;line-height:1.8;color:#ffffff;text-align:center;">${c.cta}</p></td></tr>
      <tr><td style="padding:18px 28px 22px;border-top:1px solid ${BRAND_BORDER};text-align:center;background:${BRAND_BG};"><p style="margin:0;font-size:11px;letter-spacing:.25px;color:${BRAND_MUTED};">${BRAND_NAME} &nbsp;•&nbsp; ${c.footer}</p></td></tr>
  ...
  try {
    ...
    for (const acc of accounts) {
      try {
        ...
        const subject = `NiroVera — ${getCopy(language).subject} ${acc.name || ''}`.trim();

        const msg = createMimeMessage();
        msg.setSender({ name: 'NiroVera', addr: 'no-reply@powercare.app' });
