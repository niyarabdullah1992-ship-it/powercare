...
import { validateOffboardingRequest, validationFailed } from "../../shared/proofCycleSchemas.ts";
...
Deno.serve(async (req) => {
  try {
    ...
    let body = rawBody;
    if (action === "markReturned" || action === "complete") {
      const parsed = validateOffboardingRequest(rawBody);
