...
function hasAttachedFile(value) {
  if (!value) return false;
  if (typeof value === "string") return value.trim().length > 4;
  if (Array.isArray(value)) return value.some((item) => hasAttachedFile(item));
  if (typeof value === "object") {
    return ["fileUrl", "file_url", "url", "attachment", "attachmentUrl", "scanUrl", "file"].some((key) =>
      hasAttachedFile(value[key]),
    );
  }
  return false;
}

const ATTACHED_GOV_FIELDS = [
  { kind: "iqama", expiryKeys: ["idExpiry", "iqamaExpiry"], fileKeys: ["idFileUrl", "iqamaFileUrl", "idFile", "nationalIdFile"], ar: "الهوية / الإقامة", en: "ID / Iqama" },
  { kind: "work_permit", expiryKeys: ["workPermitExpiry"], fileKeys: ["workPermitFileUrl", "workPermitFile"], ar: "رخصة العمل", en: "Work permit" },
  { kind: "passport", expiryKeys: ["passportExpiry"], fileKeys: ["passportFileUrl", "passportFile"], ar: "الجواز", en: "Passport" },
  { kind: "medical", expiryKeys: ["medicalInsuranceExpiry"], fileKeys: ["medicalInsuranceFileUrl", "medicalFile"], ar: "التأمين الطبي", en: "Medical insurance" },
];

/** Alerts only when a government file was attached AND has an expiry date. */
export function deriveAttachedGovDocAlerts(employees, today = localDateKey()) {
  const out = [];
  for (const emp of employees || []) {
    const profile = emp.profile || {};
    const employeeId = emp.id || emp.employeeId;
    const name = emp.name || profile.name || employeeId;
    const stationName = emp.stationName || profile.stationName || "";

    for (const spec of ATTACHED_GOV_FIELDS) {
      const expiryDate = spec.expiryKeys.map((key) => profile[key] || emp[key]).find(Boolean);
      if (!expiryDate) continue;
      const attached = spec.fileKeys.some((key) => hasAttachedFile(profile[key] || emp[key]))
        || (emp.docs || []).some((doc) => String(doc.kind) === spec.kind && hasAttachedFile(doc))
        || (profile.files || emp.files || []).some((file) => String(file.kind || file.type || "") === spec.kind && hasAttachedFile(file));
      if (!attached) continue;
      const days = daysUntilExpiry(String(expiryDate).slice(0, 10), today);
      if (days == null || days > EXPIRY_WARN_DAYS) continue;
      out.push({
        employeeId,
        name,
        ...
        days,
        notifyAr: ["الموظف", "الموارد البشرية", stationName ? "مدير الفرع" : null].filter(Boolean),
        notifyEn: ["Employee", "HR", stationName ? "Station manager" : null].filter(Boolean),
      });
    }

    for (const doc of emp.docs || []) {
      if (!doc.expiryDate || !hasAttachedFile(doc)) continue;
      if (ATTACHED_GOV_FIELDS.some((spec) => spec.kind === String(doc.kind))) continue;
      const days = daysUntilExpiry(doc.expiryDate, today);
      if (days == null || days > EXPIRY_WARN_DAYS) continue;
      const label = docLabel(doc.kind);
      out.push({
    ...
    const contract = profile.contract || emp.contract;
    const contractEnd = contract?.endDate || profile.contractEnd;
    if (contractEnd && hasAttachedFile(contract)) {
      const days = daysUntilExpiry(String(contractEnd).slice(0, 10), today);
      if (days != null && days <= EXPIRY_WARN_DAYS) {
        out.push({
          employeeId,
          name,
          stationName,
          kind: "contract",
          docLabelAr: "عقد العمل",
          docLabelEn: "Employment contract",
          expiryDate: String(contractEnd).slice(0, 10),
          days,
          notifyAr: ["الموظف", "الموارد البشرية", stationName ? "مدير الفرع" : null].filter(Boolean),
          notifyEn: ["Employee", "HR", stationName ? "Station manager" : null].filter(Boolean),
        });
