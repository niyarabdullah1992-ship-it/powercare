...
export function exportExcelColored({ filename, title, headers, rows, color = "#e0a43b", dir = "ltr", theme = "default" }) {
  ...
  const headerCells = headers.map((h) => `<th style="background:${ink};color:#F0C56D;padding:11px 14px;border:1px ${borderStyle} ${gold};font-size:12px;font-weight:700;">${esc(h)}</th>`).join("");
  const bodyRows = rows.map((row, index) => {
    ...
    return `<tr>${row.map((cell) => `<td style="padding:8px 12px;border:1px ${borderStyle} #D8D5CC;background:${background};color:${ink};font-size:12px;">${esc(cell)}</td>`).join("")}</tr>`;
  }).join("");
  ...
  const charts = analytics.charts.map((chart) => `<tr><td colspan="${columnCount}" style="padding:15px 16px 8px;background:#ffffff;color:${ink};font-size:14px;font-weight:700;border-top:2px solid ${gold};">${esc(chart.title)}</td></tr>${chart.entries.map((entry) => `<tr><td style="padding:7px 10px;color:${ink};font-size:11px;">${esc(entry.label)}</td><td style="width:320px;padding:7px 10px;"><div style="width:${entry.percent}%;height:14px;background:${gold};border-left:5px solid ${ink};"></div></td><td style="padding:7px 10px;color:${ink};font-weight:700;">${esc(entry.display)}</td></tr>`).join("")}`).join("");
  ...
  const html = `<html dir="${dir}" xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel"><head><meta charset="UTF-8"><style>table{border-collapse:collapse;font-family:Tahoma,Arial,sans-serif}td,th{vertical-align:middle}</style></head><body><table>${banner}${summary}${charts}<tr>${headerCells}</tr>${bodyRows}</table></body></html>`;
