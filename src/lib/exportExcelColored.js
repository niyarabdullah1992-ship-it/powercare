...
export function exportExcelColored({ filename, title, headers, rows, color = "#e0a43b", dir = "ltr", theme = "default" }) {
  ...
  const ink = "#13283D";
  const cream = glass ? "#E4EEF1" : executive ? "#F1EADC" : "#FAF8F2";
  ...
  const bodyRows = rows.map((row, index) => {
    ...
    const background = glass ? (index % 2 === 0 ? "#F7FBFC" : "#E4EEF1") : (index % 2 === 0 ? "#ffffff" : cream);
  ...
  const html = `<html dir="${dir}" xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel"><head><meta charset="UTF-8"><style>body{margin:18px;background:${glass ? "#071C2A" : "#ffffff"}}table{width:${tableWidth}px;table-layout:fixed;border-collapse:collapse;font-family:Tahoma,Arial,sans-serif}td,th{vertical-align:middle;mso-number-format:\"General\"}</style></head><body><table>${columns}${banner}${summary}${charts}<tr>${headerCells}</tr>${bodyRows}</table></body></html>`;
