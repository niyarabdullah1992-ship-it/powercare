...
export default function Complaints() {
  ...
  const openVoice = (data?.anonymousReports || []).filter((r) => r.status === "open" && canSeeReport(r)).length
    + (data?.publicReports || []).filter((r) => r.status === "open" && canSeeReport(r)).length;

  return (
    <PlatformStampShell
      ar={ar}
      kicker={erpKicker("/app/complaints", lang)}
    ...
    >
      <ErpSectionFrame
        ...
        hideProof
        stats={isManager ? [
          { label: ar ? "بلاغات مفتوحة" : "Open reports", value: openVoice, tone: openVoice > 0 ? "warn" : "ok" },
          { label: ar ? "قنوات" : "Channels", value: 3, hint: ar ? "اقتراح · شكوى · مجهول" : "Suggest · complain · anon" },
        ] : undefined}
      >
