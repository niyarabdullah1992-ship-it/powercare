  ...
  return (
    ...
    <div className="tasks-hub space-y-4">
      {!isIndividual && !checkedInToday && (
        <div className="flex flex-wrap items-center gap-3 rounded-[10px] border border-[#E4E7EC] bg-white px-4 py-3.5">
          <p className="flex-1 text-[12.8px] leading-relaxed text-[#475467]">
            {lang === "ar"
              ? "المهام الميدانية مرتبطة بدورة الإثبات: سجّل حضور اليوم أولًا ثم نفّذ وأرفق الإثبات."
              : "Field tasks sit on the proof cycle: check in today first, then execute and attach proof."}
          </p>
          <Link
            to="/app/attendance"
            className="shrink-0 rounded-lg bg-[#0B1A3F] px-3.5 py-2 text-[12.5px] text-white hover:bg-[#14233C]"
          >
            {t("goToAttendance") || (lang === "ar" ? "تسجيل الحضور" : "Check in")}
          </Link>
        </div>
      )}
      ...
      <div className="flex flex-wrap items-center gap-2.5">
        <span className="text-[12.5px] text-[#98A2B3]">
          {lang === "ar" ? "الرئيسية ← المهام ← الأقسام" : "Home ← Tasks ← Sections"}
        </span>
        <div className="ms-auto flex flex-wrap gap-2">
          {canCreateTasks(currentUser, data) && (
            <button
              type="button"
              onClick={() => openCreateForm(selectedStation, folderPath)}
              className="rounded-lg bg-[#0E7A4B] px-3.5 py-2 text-[12.5px] text-white hover:bg-[#0B5F3A]"
            >
          ...
          <button
            ...
            className={`rounded-lg border border-[#E4E7EC] bg-white px-3.5 py-2 text-[12.5px] text-[#344054] ${showArchive ? "ring-1 ring-[#0E7A4B]" : ""}`}
          >
            {lang === "ar" ? "الأرشيف الذكي" : "Smart archive"}
          </button>
          {targets.length > 0 && (
            <button
              type="button"
              onClick={() => setShowReport(!showReport)}
              className={`rounded-lg border border-[#E4E7EC] bg-white px-3.5 py-2 text-[12.5px] text-[#344054] ${showReport ? "ring-1 ring-[#0B1A3F]" : ""}`}
            >
              {lang === "ar" ? "تقرير المهام PDF / Excel" : "Tasks report PDF / Excel"}
            </button>
          )}
        </div>
      </div>

      {targets.length > 0 && showReport && (
        <TaskReportExport targets={targets} t={t} lang={lang} dir={dir} stationKeyOf={targetStationKey} defaultStation={selectedStation || "all"} />
      )}
      ...
      <div className="space-y-4 rounded-[10px] border border-[#E4E7EC] bg-white p-5">
        <div className="flex items-center justify-between gap-2 flex-wrap">
          <h2 className="text-[15px] font-semibold text-[#101828]">
            {showArchive ? t("smartArchive") : (lang === "ar" ? "المحطات والمهام" : t("targets"))}
          </h2>
        </div>

        {showArchive && (
          <p className="text-xs text-[#667085] font-body -mt-2">{t("smartArchiveHint")}</p>
        )}
        ...
        {showArchive ? (
          ...
          <div className="space-y-3 py-2" aria-label={t("loading") || "Loading"}>
            {[1, 2, 3].map((item) => <div key={item} className="h-20 rounded-lg bg-[#F2F4F7] animate-pulse" />)}
          </div>
        ) : !selectedStation ? (
          stationGroups.length === 0 ? (
            <p className="text-sm text-[#667085] font-body">{t("noTargets")}</p>
          ) : (
            <motion.div
              key="stations"
              initial={{ opacity: 0, y: 12 }}
            ...
            >
              {stationGroups.map((g) => (
                <button
                  ...
                  className="flex items-center justify-between rounded-[10px] border border-[#E4E7EC] bg-[#F9FAFB] p-4 text-start transition hover:bg-white hover:border-[#0E7A4B]/40"
                >
                  <div className="min-w-0">
                    <p className="truncate text-sm font-medium text-[#101828]">{g.name}</p>
                    <p className="mt-0.5 text-xs text-[#667085]">{g.count} {t("tasksUnit")}</p>
                  </div>
                  <ChevronRight className={`h-4 w-4 shrink-0 text-[#98A2B3] ${dir === "rtl" ? "rotate-180" : ""}`} />
                </button>
              ))}
            </motion.div>
          )
