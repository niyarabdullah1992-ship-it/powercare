...
import GlobalRolesCard from "@/components/manual/GlobalRolesCard";
...
export default function SiteManual() {
  return (
    <div dir="rtl" className="min-h-screen bg-background px-4 py-6 text-foreground md:px-8 print:bg-card print:p-0">
      ...
      <main className="manual-shell mx-auto max-w-6xl space-y-6"><ManualHeader /><ManualToc /><GlobalRolesCard /><AuditSummary />{MANUAL_CHAPTERS.map((chapter) => <ManualChapter key={chapter.id} chapter={chapter} />)}<footer className="py-6 text-center text-xs text-muted-foreground">PowerCare — الدليل التشغيلي الشامل · آخر تحديث {new Date().toLocaleDateString("ar-SA")}</footer></main>
    </div>
  );
}