...
export default function Cameras() {
  ...
  return <div className="camera-center space-y-6"><PageHeader title={ar ? "مركز العمليات الأمنية" : "Security Operations Center"} description={ar ? "إدارة التنبيهات الأمنية والكاميرات المرتبطة بالمحطات من مركز موحّد." : "Manage security alerts and station cameras from one unified center."} icon={Camera} actions={<div className="flex flex-wrap gap-2">{canManage && <><button onClick={() => setImportOpen(true)} className="inline-flex items-center gap-2 rounded-md border border-accent/30 bg-card px-3 py-2 text-sm font-semibold text-foreground hover:bg-secondary"><FileUp className="h-4 w-4" />{ar ? "استيراد CSV" : "Import CSV"}</button><button onClick={() => { setDiscoveredAddress(""); setWizardOpen(true); }} className="inline-flex items-center gap-2 rounded-md border border-accent bg-accent px-4 py-2 text-sm font-semibold text-accent-foreground hover:bg-accent/90"><Link2 className="h-4 w-4" />{ar ? "ربط كاميرا" : "Connect camera"}</button></>}</div>} />
