...
const empty = { claims: [], canManagerReview: false, canFinanceReview: false };
...
export default function Expenses() {
  ...
  return <div className="space-y-6">
    ...
    <ExpenseForm onSubmit={submit} ar={ar} />
    <ExpenseStats claims={state.claims} ar={ar} />
    {loading ? <div className="h-40 animate-pulse rounded-xl bg-muted" /> : <ExpenseList claims={state.claims} canManagerReview={state.canManagerReview} canFinanceReview={state.canFinanceReview} onManagerReview={(claimId, decision) => run("managerReview", { claimId, decision })} onFinanceReview={(claimId, decision) => run("financeReview", { claimId, decision })} ar={ar} />}
  </div>;
}