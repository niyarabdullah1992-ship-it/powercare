...
const emptyData = { items: [], requestItems: [], movements: [], requests: [], stations: [], locations: [], transferStations: [], employees: [], canManage: false, canWarehouseManage: false, canTransfer: false, centralWarehouseId: "central_warehouse" };
...
export default function Inventory() {
  ...
  const [selectedStation, setSelectedStation] = useState(currentUser?.role === "warehouse_manager" ? "" : currentUser?.stationId || "");

  const load = async () => { setLoading(true); try { setState(await inventoryCall(session, "list")); } finally { setLoading(false); } };
  useEffect(() => { load(); }, [session?.token]);
  useEffect(() => {
    if (!selectedStation && state.locations.length) setSelectedStation(currentUser?.role === "warehouse_manager" ? state.centralWarehouseId : currentUser?.stationId || state.centralWarehouseId || state.locations[0].stationId);
  }, [state.locations, state.centralWarehouseId, currentUser?.role, currentUser?.stationId, selectedStation]);
  ...
  return <div className="space-y-6">
    ...
    {!loading && <><GlobalInventorySearch items={state.items} stations={state.locations} ar={ar} onOpen={(item, stationId) => { changeStation(stationId); setActive("items"); setSelectedItem(item); }} /><StationWarehousePicker stations={state.locations} value={activeStation} onChange={changeStation} locked={currentUser?.role === "employee"} ar={ar} /></>}
    ...
    {loading ? <div className="h-40 animate-pulse rounded-xl bg-muted" /> : <>
      ...
      {active === "items" && <div className="space-y-4">{state.canManage && <ItemForm key={activeStation} items={state.items} stations={state.locations} defaultStationId={activeStation} centralWarehouseId={state.centralWarehouseId} onSubmit={(payload) => run("createItem", payload)} onTransfer={(payload) => run("transfer", payload)} ar={ar} />}<ItemList items={stationItems} stations={state.locations} onSelect={setSelectedItem} ar={ar} /></div>}
      ...
      {active === "movements" && <div className="space-y-4">{state.canTransfer && <MovementForm key={activeStation} items={stationItems} stations={state.locations} transferStations={state.transferStations} stationId={activeStation} centralWarehouseId={state.centralWarehouseId} warehouseMode={state.canWarehouseManage} onSubmit={(action, payload) => run(action, payload)} ar={ar} />}<MovementList movements={state.canWarehouseManage ? state.movements : stationMovements} items={state.requestItems} stations={state.transferStations.length ? state.transferStations : state.locations} employees={state.employees} ar={ar} /></div>}
    ...
    <ItemDetails item={selected} stations={state.locations} canDelete={state.canManage} onDelete={async (itemId) => { if (await run("deleteItem", { itemId })) setSelectedItem(null); }} onClose={() => setSelectedItem(null)} ar={ar} />
  </div>;
}