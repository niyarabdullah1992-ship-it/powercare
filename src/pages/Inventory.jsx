...
const emptyData = { items: [], requestItems: [], movements: [], purchases: [], procurementRequests: [], purchaseOrders: [], requests: [], stations: [], locations: [], transferStations: [], employees: [], canManage: false, canPurchase: false, canIssueToWork: false, canDelete: false, canApproveProcurement: false, canReceiveProcurement: false, canViewAllPurchases: false, canWarehouseManage: false, canTransfer: false, centralWarehouseId: null };
...
export default function Inventory() {
  ...
  return <div className="space-y-6">
    ...
    {loading ? <div className="h-40 animate-pulse rounded-xl bg-muted" /> : <>
      ...
      {active === "movements" && <div className="space-y-4">{state.canTransfer && <MovementForm key={`transfer-${activeStation}`} items={stationItems} stations={state.transferStations} stationId={activeStation} onSubmit={(action, payload) => run(action, payload)} ar={ar} />}<MovementList movements={stationMovements.filter((entry) => entry.movementType !== "issue")} items={state.requestItems} stations={state.transferStations.length ? state.transferStations : state.locations} employees={state.employees} ar={ar} /></div>}
    </>}
