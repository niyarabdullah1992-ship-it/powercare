...
import MapResizeHandler from "@/components/maps/MapResizeHandler";
import { resolveStationPositions } from "@/lib/geocodeStations";

const safe = (value) => String(value || "").replace(/[&<>"']/g, (char) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" }[char]));
const stationIcon = (name, status) => { const color = status === "online" ? "#22c55e" : status === "offline" ? "#ef4444" : "#d1aa58"; return divIcon({ className: "", iconSize: [160, 68], iconAnchor: [80, 21], html: `<div style="display:flex;flex-direction:column;align-items:center;width:160px;gap:4px"><div style="display:grid;place-items:center;width:42px;height:42px;border-radius:999px;border:3px solid ${color};background:hsl(var(--primary));color:${color};box-shadow:0 5px 16px rgba(0,0,0,.3)"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14.5 4h-5L7.7 7H4a2 2 0 0 0-2 2v9a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V9a2 2 0 0 0-2-2h-3.7z"/><circle cx="12" cy="13" r="3"/></svg></div><span style="padding:3px 8px;border-radius:6px;background:hsl(var(--card));color:hsl(var(--foreground));border:1px solid ${color};font:600 11px sans-serif;white-space:nowrap;box-shadow:0 2px 8px rgba(0,0,0,.18)">${safe(name)}</span></div>` }); };

export default function CameraMap({ cameras, stations, statuses = {}, ar, onSelectStation }) {
