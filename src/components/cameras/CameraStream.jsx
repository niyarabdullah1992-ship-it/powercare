...
export default function CameraStream({ camera, ar }) {
  const frameRef = useRef(null); const [state, setState] = useState("loading");
  const ready = useCallback(() => setState("online"), []); const failed = useCallback(() => setState("offline"), []);
  ...
  else if (camera.streamType === "player") media = <iframe src={camera.streamUrl} title={camera.name} className="aspect-video w-full border-0 bg-muted" allow="autoplay; fullscreen" onLoad={ready} onError={failed} />;
