...
export default function ChatGroupManager({ t, stations, groups, onAdd, onDelete }) {
  ...
  const options = stations.map((s) => ({ id: s.id, name: s.name }));
    ...
    >
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 10, flexWrap: "wrap" }}>
        ...
        <button
          ...
          style={showForm ? ui.btnCreateQuiet : ui.btnCreate}
        >
          {showForm ? t("cancel") : t("addChatGroup")}
        </button>
      </div>
          ...
          {groups.map((g) => (
            ...
            >
              <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 6, minWidth: 0 }}>
                ...
                {(g.stationIds || []).filter((id) => id && id !== "hq").map((id) => (
                  <span
                    key={id}
                    style={{
                      padding: "2px 8px",
