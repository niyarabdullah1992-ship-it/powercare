    ...
    <div className="flex-1 overflow-y-auto">
      ...
      {contacts.map((c) => (
        <div
          ...
          onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") onSelectContact(c); }}
          className={`w-full flex items-center gap-3 px-4 py-3 text-start border-b border-border/50 hover:bg-muted transition cursor-pointer ${activeChat?.type === "dm" && activeChat.userId === c.id ? "bg-muted" : ""}`}
        >
