...
function StreamVideo({ stream, muted }) { const ref = useRef(null); useEffect(() => { if (ref.current) ref.current.srcObject = stream; }, [stream]); return <video ref={ref} autoPlay playsInline muted={muted} className="h-full w-full rounded-xl bg-black object-cover" />; }
...
export default function ChatCallPanel({ call, localStream, remoteStreams, onEnd, onRecording, ar }) {
  ...
  const { recording, result, start, stop } = useCallRecorder(allStreams);
  const audioOnly = call?.mode === "audio";
  useEffect(() => { if (result) onRecording(result); }, [result]);
  if (!call) return null;
  return (
    <div className="absolute inset-0 z-30 flex flex-col bg-background p-4">
      ...
      <div className="mt-4 flex flex-wrap items-center justify-center gap-2">
        ...
        {result && <a href={URL.createObjectURL(result)} download={`call-${Date.now()}.webm`} className="flex items-center gap-2 rounded-full bg-muted px-4 py-2 text-sm"><Download className="h-4 w-4" />{ar ? "تنزيل" : "Download"}</a>}
      ...
      <p className="mt-2 text-center text-xs text-muted-foreground">{recording ? (ar ? "التسجيل فعّال لدى جهازك" : "Recording on your device") : (ar ? "يمكن لأي مشارك تسجيل المكالمة عند رغبته" : "Any participant may record when needed")}</p>
    </div>
  );
}