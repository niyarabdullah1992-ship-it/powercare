...
export default function MaterialRequestForm({ items, stations, stationId, onSubmit, ar }) {
  ...
  return <form onSubmit={submit} className="grid gap-3 rounded-xl border border-border bg-card p-4 md:grid-cols-2 xl:grid-cols-4">
    ...
    <button disabled={!sources.length || !sourceStationId} className="rounded-lg bg-accent px-4 py-2 font-medium text-accent-foreground disabled:cursor-not-allowed disabled:opacity-50 md:col-span-2 xl:col-span-4">{sources.length ? (ar ? "إرسال الطلب" : "Submit request") : (ar ? "لا يوجد رصيد في محطات أخرى" : "No stock at other stations")}</button>
  </form>;
}