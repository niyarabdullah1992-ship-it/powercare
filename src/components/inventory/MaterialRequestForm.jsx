...
import StockAvailabilityCard from "@/components/inventory/StockAvailabilityCard";
...
export default function MaterialRequestForm({ items, purchases = [], stations, stationId, onSubmit, ar }) {
  ...
  const availableQuantity = Number(selectedItem?.locationBalances?.find((balance) => balance.locationId === sourceStationId)?.quantity || 0);
  const minimumStock = Number(selectedItem?.minimumStock || 0);
  const requestedQuantity = Number(quantity || 0);
  const quantityExceedsStock = Boolean(itemId) && requestedQuantity > availableQuantity;
  ...
  const submit = async (event) => {
    ...
    if (submitting || quantityExceedsStock) return;
  ...
  return <form onSubmit={submit} className="grid gap-3 rounded-xl border border-border bg-card p-4 md:grid-cols-2 xl:grid-cols-4">
    ...
    <div className="space-y-2"><MobileSelect name="itemId" value={itemId} onChange={(value) => { setItemId(value); setQuantity("1"); }} disabled={!sourceStationId} searchable searchPlaceholder={ar ? "ابحث باسم الصنف أو الكود..." : "Search item name or code..."} placeholder={ar ? "اختر الصنف المتوفر" : "Choose available item"} className="w-full rounded-lg" options={availableItems.map((item) => ({ value: item.id, label: `${item.name} — ${ar ? "المتوفر" : "Available"}: ${Number(item.locationBalances.find((balance) => balance.locationId === sourceStationId)?.quantity || 0)}` }))} />{selectedItem && <StockAvailabilityCard balance={availableQuantity} minimumStock={minimumStock} requestedQuantity={requestedQuantity} ar={ar} />}</div>
