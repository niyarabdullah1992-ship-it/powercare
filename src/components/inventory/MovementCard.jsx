...
export default function MovementCard({ entry, itemName, stationName, personName, canReverse, onReverse, ar }) {
  ...
  const reversal = entry.isReversal || entry.movementType === "reversal";
  const reversed = !reversal && !!entry.reversedAt;
  const label = { purchase: ar ? "شراء" : "Purchase", receive: ar ? "استلام" : "Receive", issue: t("issueToWork"), return: ar ? "إرجاع" : "Return", transfer: ar ? "نقل بين المحطات" : "Station transfer", reversal: ar ? "تراجع" : "Reversal" }[entry.movementType] || entry.movementType;
  const status = { purchase: ar ? "تم تسجيل الشراء" : "Purchase recorded", receive: ar ? "تم الاستلام" : "Received", issue: ar ? "تم التسليم للعمل" : "Issued to work", return: ar ? "تم الإرجاع" : "Returned", transfer: ar ? "مكتمل" : "Completed", reversal: ar ? "تم عكس الأرصدة" : "Balances reversed" }[entry.movementType] || "—";
