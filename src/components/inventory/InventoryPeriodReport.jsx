...
export default function InventoryPeriodReport({ reportData, ar }) {
  ...
  return <div className="space-y-4">
    <button type="button" onClick={() => setOpen((value) => !value)} className={`flex items-center gap-2 rounded-full border px-4 py-2 text-sm ${open ? "border-foreground bg-foreground text-background" : "border-border bg-card hover:bg-muted"}`}><FileText className="h-4 w-4" />{ar ? "تقرير المخزون (PDF / Excel)" : "Inventory report (PDF / Excel)"}</button>
