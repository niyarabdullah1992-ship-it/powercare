...
export default function MovementRow({ entry, itemName, stationName, performerName, ar }) {
  ...
  return <Fragment><tr className="border-t">
    ...
    <td className="p-3">{["transfer", "issue"].includes(entry.movementType) && <button onClick={() => setOpen(!open)} className="rounded-lg border p-1.5"><ChevronDown className={`h-4 w-4 ${open ? "rotate-180" : ""}`} /></button>}</td>
