...
export default function ItemForm({ stations, onSubmit, ar }) {
  ...
  return <form onSubmit={submit} className="grid gap-3 rounded-xl border border-border bg-card p-4 md:grid-cols-2 xl:grid-cols-4">
    ...
    <select name="locationId" required className="rounded-lg border px-3 py-2"><option value="">{ar ? "المحطة الحالية" : "Current station"}</option>{stations.map((station) => <option key={station.stationId} value={station.stationId}>{station.name}</option>)}</select>
