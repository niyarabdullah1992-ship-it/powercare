...
export default function InventoryWorkflow({ onNavigate, ar }) {
  ...
  return <section className="rounded-xl border border-accent/30 bg-card px-4 py-7 shadow-soft md:px-7 md:py-8">
    <div className="mb-9 text-center"><p className="font-mono text-[10px] uppercase tracking-widest text-accent">PowerCare Workflow</p><h2 className="mt-2 font-heading text-3xl font-bold md:text-4xl">{ar ? "دورة حركة الصنف" : "Item movement cycle"}</h2><p className="mt-3 text-lg text-foreground/70 md:text-xl">{ar ? "تبدأ بالشراء وتنتهي عند صرف الصنف للعمل." : "Starts with purchase and ends when the item is issued to work."}</p></div>
