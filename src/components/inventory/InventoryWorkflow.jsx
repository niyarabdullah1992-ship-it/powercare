...
export default function InventoryWorkflow({ canManage, onNavigate, ar }) {
  ...
  return <section className="rounded-2xl border border-border bg-card px-5 py-10 md:px-10 md:py-12">
    ...
    <div className="grid gap-x-9 gap-y-14 [direction:ltr] md:grid-cols-3">{steps.map(([key, Icon, title, text], index) => <button type="button" key={key} onClick={() => onNavigate(key)} className="relative flex min-h-64 flex-col items-center justify-center rounded-xl border-2 border-accent/70 bg-card px-5 pb-7 pt-11 text-center [direction:rtl] hover:border-accent"><span className="absolute left-1/2 top-0 flex h-14 w-14 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full bg-accent text-xl font-bold text-accent-foreground shadow-sm">{index + 1}</span><Icon className="mb-6 h-12 w-12 text-accent" strokeWidth={1.6} /><span className="block text-xl font-bold leading-snug">{title}</span><span className="mt-3 block max-w-56 text-base leading-7 text-foreground/80">{text}</span></button>)}</div>
  </section>;
}