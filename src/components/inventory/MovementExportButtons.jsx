...
export default function MovementExportButtons({ movements, items, stations, employees = [], ar }) {
  ...
  const rows = movements.map((entry) => [new Date(entry.created_date).toLocaleString(ar ? "ar-SA" : "en"), item(entry.itemId), type(entry.movementType), status(entry.movementType), entry.quantity, station(entry.fromLocationId), station(entry.toLocationId), person(entry.employeeId), person(entry.performedBy), entry.balanceBefore ?? "—", entry.balanceAfter ?? "—", entry.destinationBalanceBefore ?? "—", entry.destinationBalanceAfter ?? "—"]);
