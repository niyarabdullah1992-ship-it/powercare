...
export default function MovementForm({ items, stations, transferStations = stations, stationId, centralWarehouseId, onSubmit, ar }) {
  ...
  return <form onSubmit={submit} className="grid gap-3 rounded-xl border border-border bg-card p-4 md:grid-cols-2 xl:grid-cols-4">
    <select value={type} onChange={(event) => setType(event.target.value)} className="rounded-lg border px-3 py-2"><option value="receive">{ar ? "استلام" : "Receive"}</option><option value="return">{ar ? "إرجاع للمستودع" : "Return to warehouse"}</option><option value="transfer">{ar ? "نقل بين المحطات" : "Station transfer"}</option></select>
    ...
    {type === "receive" && <input type="hidden" name="toLocationId" value={stationId} />}
