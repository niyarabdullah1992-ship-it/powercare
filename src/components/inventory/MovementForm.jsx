...
export default function MovementForm({ items, stations, stationId, onSubmit, ar }) {
  ...
  return <form onSubmit={submit} className="grid gap-3 rounded-xl border border-border bg-card p-4 md:grid-cols-2 xl:grid-cols-4">
    ...
    {type === "transfer" && <select name="fromLocationId" required defaultValue={stationId || ""} className="rounded-lg border px-3 py-2"><option value="">{ar ? "من محطة" : "From station"}</option>{stations.map((station) => <option key={station.stationId} value={station.stationId}>{station.name}</option>)}</select>}
    <select name="toLocationId" required defaultValue={type === "transfer" ? "" : stationId || ""} className="rounded-lg border px-3 py-2"><option value="">{ar ? "إلى محطة" : "To station"}</option>{stations.map((station) => <option key={station.stationId} value={station.stationId}>{station.name}</option>)}</select>
