...
export default function ItemDetails({ item, stations, canDelete, onDelete, onImagesChange, onClose, ar }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}><div onClick={(event) => event.stopPropagation()} className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-card p-5">
    ...
    {canDelete && <button type="button" onClick={() => { if (window.confirm(ar ? "هل تريد حذف الصنف نهائياً؟ سيتم حذف جميع الحركات والطلبات المرتبطة به ولا يمكن التراجع." : "Permanently delete this item? All related movements and requests will be deleted and this cannot be undone.")) onDelete(item.id); }} className="mt-5 flex w-full items-center justify-center gap-2 rounded-lg border border-destructive/30 px-4 py-2 text-destructive hover:bg-destructive/10"><Trash2 className="h-4 w-4" />{ar ? "حذف الصنف نهائياً" : "Delete item permanently"}</button>}
  </div></div>;
}