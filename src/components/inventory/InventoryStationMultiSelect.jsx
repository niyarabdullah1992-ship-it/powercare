...
export default function InventoryStationMultiSelect({ stations, value, onChange, ar }) {
  ...
  const label = !value.length
    ...
    : selected.length === 1
      ? selected[0].name
      : (ar ? `${selected.length} ${terms.stations.replace(/^ال/, "")}` : `${selected.length} ${terms.stations.toLowerCase()}`);
