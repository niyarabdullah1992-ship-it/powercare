...
export default function RequestsList({ requests, items, employees, stations, centralWarehouseId, canManage, onReview, onIssue, ar }) {
  ...
  const available = (request) => Number(item(request.itemId)?.locationBalances?.find((balance) => balance.locationId === request.sourceStationId)?.quantity || 0);
  return <div className="space-y-3">{requests.map((request) => <div key={request.id} className="rounded-xl border border-border bg-card p-4">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-semibold">{item(request.itemId)?.name || "—"} · {request.quantity}</p><p className="text-xs text-muted-foreground">{stationName(request.sourceStationId)} ← {stationName(request.stationId)} · {employeeName(request.requesterId)}{request.notes ? ` — ${request.notes}` : ""}</p>{canManage && <p className="mt-1 text-xs font-medium text-accent">{ar ? "المتاح في محطة المصدر" : "Available at source"}: {available(request)}</p>}</div><span className={`rounded-full px-2 py-1 text-[10px] ${badge[request.status]}`}>{request.status}</span></div>
