...
export default function CheckInOutCard({ currentUser, company, t, onStatusChange, compact = false }) {
  ...
  const punchButton = (action, label) => (
    <button
      ...
      disabled={loading || (action === "in" && (scheduleBlocks && !localMode || onLeaveToday))}
      style={{
        ...ui.btnPrimary,
        ...
        alignItems: "center",
        gap: 7,
        opacity: loading || (action === "in" && ((scheduleBlocks && !localMode) || onLeaveToday)) ? 0.45 : 1,
        cursor: loading || (action === "in" && ((scheduleBlocks && !localMode) || onLeaveToday)) ? "not-allowed" : "pointer",
      }}
    >
      {loading
      ...
      <div style={{ padding: "14px 16px 16px", display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        {onLeaveToday && (
          <p style={{ margin: 0, fontSize: 11, color: "#1D4ED8" }}>
            {ar ? "إجازة معتمدة اليوم — البصمة غير متاحة." : "Approved leave today — check-in is disabled."}
          </p>
        )}
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {phase === "awaiting_in" && (
            <button
              type="button"
              onClick={() => punch("in")}
              disabled={loading || ((scheduleBlocks && !localMode) || onLeaveToday)}
              style={{
                ...ui.btnPrimary,
                height: 40,
                padding: "0 18px",
                ...
                alignItems: "center",
                gap: 7,
                opacity: loading || ((scheduleBlocks && !localMode) || onLeaveToday) ? 0.45 : 1,
                cursor: loading || ((scheduleBlocks && !localMode) || onLeaveToday) ? "not-allowed" : "pointer",
              }}
            >
