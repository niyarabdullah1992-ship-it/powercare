...
export default function AttendanceEmergencyPanel({ company }) {
  const { lang } = useI18n();
  const ar = lang === "ar";
  const [window, setWindow] = useState({ start: "", end: "", active: false });
  const [saving, setSaving] = useState(false);
  const [message, setMessage] = useState("");

  const load = () => base44.functions.invoke("supabaseAttendance", { action: "getSettings", companyId: company.id }).then(({ data }) => setWindow({ start: toLocalInput(data?.settings?.emergency_start_at), end: toLocalInput(data?.settings?.emergency_end_at), active: !!data?.settings?.emergency_active }));
  useEffect(() => { load(); }, [company.id]);

  const save = async (event) => {
    event.preventDefault(); setSaving(true); setMessage("");
    try {
      await base44.functions.invoke("supabaseAttendance", { action: "setAttendanceEmergency", companyId: company.id, startAt: new Date(window.start).toISOString(), endAt: new Date(window.end).toISOString() });
      await load(); setMessage(ar ? "تم حفظ فترة الاستثناء." : "Emergency window saved.");
    } catch (error) { setMessage(error?.response?.data?.error || (ar ? "تعذر حفظ الاستثناء." : "Unable to save the emergency window.")); }
  ...
  return <form onSubmit={save} className="space-y-4 rounded-xl border border-amber-300 bg-amber-50/60 p-5">
    ...
    <div className="grid gap-3 sm:grid-cols-2"><label className="text-xs text-muted-foreground">{ar ? "وقت البداية" : "Start time"}<input required type="datetime-local" value={window.start} onChange={(e) => setWindow({ ...window, start: e.target.value })} className="mt-1 w-full rounded-md border px-3 py-2 text-sm" /></label><label className="text-xs text-muted-foreground">{ar ? "وقت النهاية" : "End time"}<input required type="datetime-local" value={window.end} onChange={(e) => setWindow({ ...window, end: e.target.value })} className="mt-1 w-full rounded-md border px-3 py-2 text-sm" /></label></div>
