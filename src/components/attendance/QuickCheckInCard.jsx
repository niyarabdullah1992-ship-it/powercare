...
export default function QuickCheckInCard({ currentUser, company }) {
  ...
  const handleCheckIn = async () => {
    ...
    try {
      ...
      setError(code === "NOT_SCHEDULED" ? (ar ? "لا يمكنك تسجيل الحضور لأنك غير مدرج في جدول اليوم." : "You cannot check in because you are not scheduled today.") : code === "GPS_REQUIRED" ? t("locationDenied") : code === "STATION_LOCATION_REQUIRED" ? t("locationNotSet") : code === "OUTSIDE_STATION" ? t("outsideLocation") : (code || t("aiActionFailed")));
    } finally {
      setLoading(false);
    }
  };
