...
export default function ComparisonReportPage({ page, total }) {
  return <article data-pdf-page dir="rtl" className="relative mx-auto flex h-[1123px] w-[794px] shrink-0 flex-col overflow-hidden bg-background px-11 py-10 text-foreground shadow-elevated">
    ...
    <div className="overflow-hidden rounded-lg border border-border bg-card">
      <div className="grid grid-cols-[1.05fr_1.55fr_1.55fr_.9fr] bg-primary px-3 py-3 text-[10px] font-bold text-primary-foreground"><span>المعيار</span><span>PowerCare</span><span>SAP التقليدي</span><span>النتيجة</span></div>
      {page.rows.map((row, index) => <div key={`${page.number}-${index}`} className="grid grid-cols-[1.05fr_1.55fr_1.55fr_.9fr] border-t border-border px-3 py-2.5 text-[11px] leading-5"><strong>{row[0]}</strong><span>{row[1]}</span><span className="text-muted-foreground">{row[2]}</span><span className="font-semibold text-accent-foreground">{row[3]}</span></div>)}
    </div>
