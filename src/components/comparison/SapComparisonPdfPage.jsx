...
export default function SapComparisonPdfPage({ number, eyebrow, title, children }) {
  return (
    <article data-pdf-page dir="rtl" className="relative mx-auto flex h-[1123px] w-[794px] shrink-0 flex-col overflow-hidden bg-background px-11 py-10 text-foreground shadow-elevated">
      ...
      <header className="flex items-center justify-between border-b border-accent/30 pb-5"><div className="flex items-center gap-3"><Logo size={38} wordmark={false} /><div><p className="font-heading text-lg font-bold">NiroVera</p><p className="text-[8px] tracking-[.2em] text-muted-foreground">SAP PAIN-TO-SOLUTION</p></div></div><span className="font-mono text-xs text-accent">{number} / 04</span></header>
