...
export default function TrialExpiryGate({ company, children }) {
  ...
  useEffect(() => {
    ...
    base44.functions.invoke("companyDirectory", { action: "accountExists", companyId: company.id })
      .then((res) => setAccess(res.data)).catch(() => setAccess({ exists: true }));
  }, [company?.id]);
  if (company?.id && access === null) return <div className="flex min-h-screen items-center justify-center bg-background"><Loader2 className="h-6 w-6 animate-spin text-accent" /></div>;
