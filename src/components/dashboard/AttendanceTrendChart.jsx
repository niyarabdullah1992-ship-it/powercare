  ...
  return (
    ...
    <div className="holo-chart rounded-2xl p-5 sm:p-6">
      <div className="mb-5 flex items-center justify-between">
        ...
        <span className="rounded-full border border-holo-green/40 px-3 py-1 text-xs font-body text-holo-green">
          {t("sixMonthsLabel")}
        </span>
      </div>
      <ResponsiveContainer width="100%" height={300}>
        <LineChart data={data}>
          <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--holo-green) / .16)" vertical={false} />
          ...
          <Line
            ...
            dataKey="completed"
            name={t("completed")}
            stroke="hsl(var(--holo-green))"
            strokeWidth={3}
            dot={{ r: 4, fill: "hsl(var(--holo-green))", stroke: "hsl(var(--holo-green-soft))", strokeWidth: 2 }}
          />
          <Line
            type="natural"
            dataKey="pending"
            name={t("pending")}
            stroke="hsl(var(--holo-green-soft))"
            strokeWidth={2.5}
            strokeDasharray="7 5"
            dot={{ r: 3.5, fill: "hsl(var(--holo-green-soft))", strokeWidth: 0 }}
          />
        </LineChart>
      </ResponsiveContainer>
    </div>
