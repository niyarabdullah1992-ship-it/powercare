  ...
  return (
    <section className="rounded-2xl border border-white/10 bg-white/5 p-5 shadow-sm">
      ...
      {risks.length === 0 ? <p className="rounded-xl bg-emerald-500/10 p-4 text-sm text-emerald-700">{ar ? "لا توجد مخاطر تشغيلية مرتفعة الآن." : "No elevated operational risks detected."}</p> : <div className="space-y-2">
        {risks.map(({ icon: Icon, count, label, to, level }) => <Link key={label} to={to} className="flex items-center gap-3 rounded-xl border border-white/10 bg-black/10 p-3 hover:bg-white/10"><span className={`flex h-9 w-9 items-center justify-center rounded-lg ${level === "high" ? "bg-destructive/10 text-destructive" : "bg-accent/10 text-accent"}`}><Icon className="h-4 w-4" /></span><div className="min-w-0 flex-1"><p className="text-sm font-medium">{label}</p><p className="text-xs text-white/50">{count} {ar ? "حالة مرصودة" : "signals detected"}</p></div></Link>)}
      </div>}
    </section>
  );
}