...
export default function OperationalAlerts({ alerts, loading, lang }) {
  ...
  const ar = lang === "ar";
  return (
    <section className="h-full rounded-2xl border border-ops-border bg-ops-surface p-5 shadow-sm sm:p-6">
      ...
      {loading ? <div className="h-16 animate-pulse rounded-xl bg-muted" /> : alerts.length === 0 ? <p className="rounded-xl bg-emerald-500/10 px-4 py-3 text-sm text-emerald-700">{ar ? "لا توجد مشاكل تشغيلية حالياً." : "No operational issues right now."}</p> : (
        <div className="grid gap-3 sm:grid-cols-2">{alerts.map((alert) => { const Icon = icons[alert.category] || AlertTriangle; return <button key={alert.id} onClick={() => navigate(alert.to)} className="flex items-center gap-3 rounded-xl border border-ops-border bg-ops-bg/60 p-4 text-start hover:border-ops-gold"><Icon className="h-5 w-5 shrink-0 text-amber-700" /><span className="min-w-0"><strong className="block text-2xl leading-none">{alert.count}</strong><span className="mt-1 block text-xs text-muted-foreground">{labels[ar ? "ar" : "en"][alert.category]}</span></span></button>; })}</div>
      )}
    </section>
  );
}