...
export default function ManualHeader({ meta, labels }) {
  return (
    <header className="rounded-3xl bg-primary p-6 text-primary-foreground md:p-10 print:rounded-none">
      <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
        ...
        <button onClick={() => window.print()} className="no-print flex shrink-0 items-center justify-center gap-2 rounded-full bg-accent px-5 py-3 text-sm font-semibold text-accent-foreground"><Download className="h-4 w-4" /> {labels.download}</button>
      </div>
    </header>
  );
}