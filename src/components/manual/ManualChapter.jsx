...
export default function ManualChapter({ chapter, labels, lang, exportMode }) {
  ...
  return (
    <section id={chapter.id} className="manual-chapter scroll-mt-24 rounded-2xl border border-accent/20 bg-card p-5 shadow-soft md:p-7">
      ...
      <p className="rounded-xl border border-accent/20 bg-accent/5 p-4 text-sm leading-7 text-foreground/80">{chapter.purpose}</p>
      ...
      <div className="mt-5"><ManualBrowserShot route={chapter.route} title={chapter.name} captureLabel={labels.screenshot} language={lang} forceActive={exportMode} /></div>
      ...
      <ManualActions actions={chapter.actions || [{ title: labels.steps, steps: chapter.steps, note: chapter.roles?.[0] }]} title={labels.actions || labels.controls} />
