...
const dropModes = ["above", "left", "right", "inside", "below"];
...
export default function FlexOrgBranch({ node, nodes, data, escalationChain, canManage, dragging, actions, ar }) {
  ...
  const allowedModes = !dragging || dragging === node.id ? [] : isDescendant(nodes, node.id, dragging) ? ["inside", "below"] : dropModes;
  ...
  return <div className="flex min-w-max flex-col items-center">
    <FlexOrgCard node={node} label={label} canManage={canManage} dragging={dragging} complaintLevel={node.type === "employee" ? escalationChain.indexOf(node.refId) + 1 : 0} ar={ar} childrenCount={children.length} collapsed={collapsed} allowedModes={allowedModes} onToggleCollapse={() => setCollapsed((value) => !value)} onToggleEscalation={() => actions.toggleEscalation(node.refId)} onDragStart={actions.start} onDragEnd={actions.end} onDrop={actions.drop} onEdit={actions.edit} />
