...
export default function FlexOrgBranch({ node, nodes, data, escalationChain, canManage, dragging, actions, ar }) {
  ...
  return <div className="flex min-w-max flex-col items-center">
    ...
    {!collapsed && children.length > 1 && <><div className="h-8 w-px bg-accent/40" /><div className="relative flex items-start justify-center gap-8 pt-8">{children.map((child, index) => <div key={child.id} className="relative before:absolute before:-top-8 before:left-1/2 before:h-8 before:w-px before:bg-accent/35">{index > 0 && <div className="absolute -top-8 -left-4 right-1/2 h-px bg-accent/35" />}{index < children.length - 1 && <div className="absolute -top-8 left-1/2 -right-4 h-px bg-accent/35" />}<FlexOrgBranch node={child} nodes={nodes} data={data} escalationChain={escalationChain} canManage={canManage} dragging={dragging} actions={actions} ar={ar} /></div>)}</div></>}
  </div>;
}