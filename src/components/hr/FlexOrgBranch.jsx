...
export default function FlexOrgBranch({ node, nodes, data, escalationChain, canManage, dragging, actions, ar, lang }) {
  ...
  const firstStationEmployee = station ? children.find((child) => child.type === "employee") : null;
  const stationManager = firstStationEmployee
    ? data.employees.find((item) => item.id === firstStationEmployee.refId)
    : station?.managerId ? data.employees.find((item) => item.id === station.managerId) : null;
  const parent = nodes.find((item) => item.id === node.parentId);
  const firstEmployeeUnderParent = parent?.type === "station"
    ? nodes.filter((item) => item.parentId === parent.id && item.type === "employee").sort((a, b) => a.order - b.order)[0]
    : null;
  const isStationLead = Boolean(employee && firstEmployeeUnderParent?.id === node.id);
