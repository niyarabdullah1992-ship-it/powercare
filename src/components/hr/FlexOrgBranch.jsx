...
export default function FlexOrgBranch({ node, nodes, data, escalationChain, canManage, dragging, actions, ar }) {
  ...
  return <div className="flex min-w-max flex-col items-center">
    ...
    {!collapsed && children.length > 1 && <><div className="h-8 w-px bg-accent/40" /><div className="relative flex items-start justify-center gap-8 overflow-visible px-6 pt-8">{children.map((child, index) => <div key={child.id} className="relative before:absolute before:-top-8 before:left-1/2 before:h-8 before:w-px before:bg-accent/35">{index > 0 && <span aria-hidden="true" className="pointer-events-none absolute -left-4 -top-8 right-1/2 h-px bg-accent/35" />}{index < children.length - 1 && <span aria-hidden="true" className="pointer-events-none absolute -right-4 -top-8 left-1/2 h-px bg-accent/35" />}<FlexOrgBranch node={child} nodes={nodes} data={data} escalationChain={escalationChain} canManage={canManage} dragging={dragging} actions={actions} ar={ar} /></div>)}</div></>}
  </div>;
}