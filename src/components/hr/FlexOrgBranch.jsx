...
export default function FlexOrgBranch({ node, nodes, data, escalationChain, canManage, dragging, actions, ar, lang }) {
  ...
  const isStationManager = Boolean(employee && data.stations.some((item) => item.managerId === employee.id));
  const isHierarchyManager = Boolean(employee && children.some((child) => child.type === "employee"));
  const draggingNode = nodes.find((item) => item.id === dragging);
  const reorderActive = Boolean(draggingNode && draggingNode.id !== node.id && (draggingNode.parentId || null) === (node.parentId || null));
  ...
  return <div className="flex min-w-max flex-col items-center">
    <FlexOrgCard node={node} label={label} stationManagerName={stationManager?.name} isStationManager={isStationManager} isHierarchyManager={isHierarchyManager} canManage={canManage} dragging={dragging} reorderActive={reorderActive} complaintLevel={node.type === "employee" ? escalationChain.indexOf(node.refId) + 1 : 0} ar={ar} childrenCount={children.length} collapsed={collapsed} onToggleCollapse={() => setCollapsed((value) => !value)} onToggleEscalation={() => actions.toggleEscalation(node.refId)} onDragStart={actions.start} onDragEnd={actions.end} onDrop={actions.drop} onEdit={actions.edit} />
