...
export default function FlexOrgTree({ data, company, currentUser, lang }) {
  ...
  const actions = { start: setDragging, end: () => setDragging(null), edit: setEditing, toggleEscalation: (employeeId) => toggleComplaintEscalationMember(company.id, employeeId), drop: (targetId, mode, sourceId) => { moveOrgNode(company.id, sourceId || dragging, targetId, mode); setDragging(null); } };
  return <section ref={sectionRef} className={`${fullscreen ? "fixed inset-0 z-[70] flex h-screen w-screen flex-col rounded-none" : "rounded-xl"} overflow-hidden border border-accent/30 bg-card shadow-sm`} dir={ar ? "rtl" : "ltr"}><header className="flex flex-wrap items-center justify-between gap-3 border-b border-accent/20 bg-primary px-4 py-4 text-primary-foreground"><div className="flex items-center gap-3"><span className="rounded-lg bg-accent/15 p-2"><Network className="h-5 w-5 text-accent" /></span><div><h2 className="font-heading text-xl font-semibold">{ar ? "الشجرة التنظيمية المرنة" : "Flexible organization tree"}</h2><p className="text-[11px] text-primary-foreground/70">{ar ? "اسحب العقدة واختر موضعاً واضحاً: بينها وبين الأصل، تابعة لها، أو بعدها" : "Drag a node and choose: between parent, as child, or after"}</p></div></div><div className="flex items-center gap-2"><OrgTreeFullscreenButton active={fullscreen} onToggle={toggleFullscreen} targetRef={sectionRef} ar={ar} /><HierarchyZoomControls zoom={zoom} onZoom={(change) => setSafeZoom(zoom + change)} onSetZoom={setSafeZoom} onFit={fitTree} ar={ar} />{canManage && <button onClick={() => setEditing(null)} className="flex items-center gap-2 rounded-md bg-accent px-3 py-2 text-xs font-semibold text-accent-foreground"><Plus className="h-4 w-4" />{ar ? "إضافة" : "Add"}</button>}</div></header>
    <OrgTreeGuide ar={ar} />
