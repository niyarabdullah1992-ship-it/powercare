...
export default function FlexOrgTree({ data, company, currentUser, lang }) {
  ...
  const actions = { start: setDragging, end: () => setDragging(null), edit: setEditing, toggleEscalation: (employeeId) => toggleComplaintEscalationMember(company.id, employeeId), drop: (targetId, mode) => { if (mode === "cancel") { setDragging(null); return; } const resolvedMode = ar && mode === "left" ? "right" : ar && mode === "right" ? "left" : mode; moveOrgNode(company.id, dragging, targetId, resolvedMode); setDragging(null); } };
