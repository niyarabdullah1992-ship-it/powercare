...
import { Building2, ChevronDown, ChevronRight, GripVertical, ShieldCheck, UserCog, UserRound, Users } from "lucide-react";
...
export default function FlexOrgCard({ node, employee, label, canManage, dragging, complaintLevel, childrenCount, collapsed, onToggleCollapse, onToggleEscalation, onDragStart, onDragEnd, onDrop, onEdit, ar }) {
  ...
  const openNode = () => station ? onEdit(node) : !department && employee?.id && navigate(`/app/employees/${employee.id}`);
  ...
  return <div className="relative mx-auto w-56" onDragEnter={(event) => event.preventDefault()}>
    ...
    <div role="button" tabIndex={0} {...touchDrag.handlers} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") openNode(); }} onClick={(event) => { if (touchDrag.suppressClick()) event.preventDefault(); else openNode(); }} className={`w-full cursor-grab select-none rounded-lg border p-3 text-start transition active:cursor-grabbing ${station ? "border-accent/50 bg-primary text-primary-foreground shadow-md" : department ? "border-accent bg-accent/15 text-foreground shadow-md" : "border-border bg-card shadow-sm hover:border-accent/60"}`}>
      <span className="flex items-center gap-2.5">
        ...
        <span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold">{label}</span>{!department && <span className={`mt-0.5 flex items-center gap-1 truncate text-[10px] ${station ? "text-primary-foreground/65" : "text-muted-foreground"}`}>{!station && <ShieldCheck className="h-3 w-3" />}{node.title || (ar ? "بدون مسمى" : "Untitled")}</span>}</span>
      </span>
    </div>
