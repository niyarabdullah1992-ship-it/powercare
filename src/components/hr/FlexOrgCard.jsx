...
import { Building2, ChevronDown, ChevronRight, Crown, GripVertical, ShieldCheck, Star, UserRound } from "lucide-react";
...
export default function FlexOrgCard({ node, employee, label, rank, canManage, dragging, complaintLevel, childrenCount, collapsed, onToggleCollapse, onToggleEscalation, onDragStart, onDragEnd, onDrop, onEdit, ar }) {
  const station = node.type === "station";
  const touchDrag = useOrgNodeDrag(node.id, canManage, onDragStart, onDragEnd, onDrop);
  const RankIcon = rank === "manager" ? Crown : rank === "supervisor" ? Star : rank === "lead" ? ShieldCheck : null;
  const rankStyle = rank === "manager" ? "border-2 border-accent bg-primary text-primary-foreground shadow-elevated" : rank === "supervisor" ? "border-2 border-accent bg-card shadow-elevated" : rank === "lead" ? "border-2 border-primary/50 bg-secondary shadow-md" : "border-border bg-card shadow-sm hover:border-accent/60";
  return <div className={`relative mx-auto ${rank === "manager" ? "w-64" : "w-56"}`} onDragEnter={(event) => event.preventDefault()}>
    {!station && <ComplaintEscalationBadge level={complaintLevel} canManage={canManage} ar={ar} onToggle={onToggleEscalation} />}
    <div role="button" tabIndex={0} draggable={canManage} {...touchDrag.handlers} onDragStart={(event) => { event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData("text/plain", node.id); onDragStart(node.id); }} onDragEnd={onDragEnd} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") onEdit(node); }} onClick={(event) => { if (touchDrag.suppressClick()) event.preventDefault(); else onEdit(node); }} className={`relative w-full cursor-grab select-none rounded-lg border p-3 text-start transition active:cursor-grabbing ${station ? "border-accent/50 bg-primary text-primary-foreground shadow-md" : rankStyle}`}>
      {RankIcon && <span className="absolute -top-3 end-3 flex h-7 w-7 items-center justify-center rounded-full border border-accent bg-card text-accent shadow-md"><RankIcon className="h-4 w-4" /></span>}
      <span className="flex items-center gap-2.5">
        ...
        <span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold">{label}</span><span className={`mt-0.5 flex items-center gap-1 truncate text-[10px] ${station || rank === "manager" ? "text-primary-foreground/65" : "text-muted-foreground"}`}>{!station && <ShieldCheck className="h-3 w-3" />}{node.title || (ar ? "بدون مسمى" : "Untitled")}</span></span>
      </span>
    </div>
