...
import { createOrgRecord, deleteOrgNode, nodeAccess, saveOrgNode } from "@/lib/orgTree";
...
export default function OrgTreeNodeModal({ initial, data, company, companyId, lang, onClose }) {
  ...
  const submit = (event) => {
    ...
    if (initial) saveOrgNode(companyId, { ...initial, title: title.trim() }, permissions);
    else {
      ...
      createOrgRecord(companyId, { ...form, email, type, title: title.trim() }, permissions);
    }
    onClose();
  };
