...
import { createOrgRecord, deleteOrgNode, nodeAccess, saveOrgNode, saveOrgStationLocation } from "@/lib/orgTree";
...
export default function OrgTreeNodeModal({ initial, data, company, companyId, lang, onClose }) {
  ...
  const submit = (event) => {
    ...
    if (initial) {
      saveOrgNode(companyId, { ...initial, title: title.trim() }, permissions);
      if (type === "station") saveOrgStationLocation(companyId, refId, mapLocation);
    } else {
      ...
      createOrgRecord(companyId, { ...form, ...mapLocation, email, type, title: title.trim() }, permissions);
    }
    onClose();
  };
