...
export default function HRFullHierarchyMap({ data, company, currentUser, stations, t, lang, onSelectStation, canReorder, onStationDragEnd }) {
  const ar = lang === "ar";
  const levels = groupLevelsByOrder(data.hrLevels || []).filter((group) => (group.manager?.active ?? true) || (group.assistant?.active ?? true)).reverse();
  const owner = data.employees.find((employee) => employee.role === "owner" || employee.email === company?.ownerEmail) || currentUser;
