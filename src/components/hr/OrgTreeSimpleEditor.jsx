...
export default function OrgTreeSimpleEditor({ node, data, companyId, ar, onClose }) {
  ...
  const hireStations = workplaceStations(stations);
  const currentHome = stations.find((s) => String(s.id) === String(currentStationId));
  const branchOptions = currentHome && isManagerUnit(currentHome) && !hireStations.some((s) => s.id === currentHome.id)
    ? [currentHome, ...hireStations]
    : hireStations;
