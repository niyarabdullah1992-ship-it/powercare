...
export default function HROrgStationCard({ station, employees, ar }) {
  ...
  return <section className="w-60 rounded-xl border border-accent/35 bg-card p-3 shadow-sm">
    <div className="text-center"><span className="mx-auto flex h-9 w-9 items-center justify-center rounded-full bg-primary text-accent"><Building2 className="h-4 w-4" /></span><h4 className="mt-2 truncate font-heading text-base font-semibold">{station.name}</h4>{station.location && <p className="mt-0.5 flex items-center justify-center gap-1 truncate text-[10px] text-muted-foreground"><MapPin className="h-3 w-3" />{station.location}</p>}</div>
