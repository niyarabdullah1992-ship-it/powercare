...
const FreeOrgCanvas = forwardRef(function FreeOrgCanvas({ nodes, data, savedPositions, escalationChain, canManage, ar, zoom, onMove, onEdit, onToggleEscalation }, ref) {
  const automatic = useMemo(() => orgVisualLayout(nodes), [nodes]);
  const drag = useFreeOrgDrag(zoom, onMove);
  ...
  return <div ref={ref} className="relative origin-top-left" style={{ width, height, zoom }} dir="ltr">
    ...
    {nodes.map((node) => {
      ...
      return <div key={node.id} className="absolute" style={{ left: point.x, top: point.y }}><FreeOrgNodeCard node={node} label={source?.name || node.title} canManage={canManage} complaintLevel={node.type === "employee" ? escalationChain.indexOf(node.refId) + 1 : 0} ar={ar} active={drag.activeId === node.id} dragHandlers={drag.handlersFor(node.id, point, canManage)} suppressClick={drag.suppressClick} onEdit={onEdit} onToggleEscalation={() => onToggleEscalation(node.refId)} /></div>;
    })}
  </div>;
});
