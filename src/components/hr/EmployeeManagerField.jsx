...
export default function EmployeeManagerField({ value, onChange, placement, onPlacementChange, stations, ar }) {
  ...
  const options = [
    ...
    ["single", ar ? "تحت المحطة" : "Below the station", value.length !== 1],
    ["group", ar ? "فوق مجموعة المحطات" : "Above station group", value.length < 2],
  ];
  return (
    <fieldset className="space-y-3 rounded-md border border-border p-3">
      ...
      <div className="space-y-1"><p className="text-[11px] font-medium text-muted-foreground">{ar ? "موضع المدير في الشجرة (اختياري)" : "Manager position in tree (optional)"}</p><div className="flex flex-wrap gap-2">{options.map(([id, label, disabled]) => <button key={id} type="button" disabled={disabled} onClick={() => onPlacementChange(id)} className={`rounded-md border px-3 py-1.5 text-xs ${placement === id ? "border-accent bg-accent/10 text-accent" : "border-border"} disabled:opacity-35`}>{label}</button>)}</div></div>
    </fieldset>
  );
}