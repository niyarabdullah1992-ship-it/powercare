    ...
    <div className="relative overflow-hidden rounded-2xl border border-border bg-card/55 shadow-2xl backdrop-blur-2xl">
      ...
      <div className="-mt-12 flex flex-col items-center px-5 pb-7 text-center">
        <div className="relative h-24 w-24 shrink-0">
          ...
          {canEdit && <button type="button" onClick={() => avatarInput.current?.click()} disabled={uploading === "avatarUrl"} className="absolute -bottom-1 -end-1 rounded-full bg-accent p-2 text-accent-foreground shadow-lg disabled:opacity-60" title={t("uploadPhoto")}>{uploading === "avatarUrl" ? <Loader2 className="h-3 w-3 animate-spin" /> : <Camera className="h-3 w-3" />}</button>}
