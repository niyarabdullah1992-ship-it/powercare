    ...
    <div className="rounded-xl border border-border bg-card p-5 space-y-5 max-w-2xl">
      <div className="flex items-center justify-between">
        <h3 className="font-heading text-lg font-semibold flex items-center gap-2">
          ...
          {text("هوية التقارير وملفات PDF", "Reports & PDF branding", "レポートとPDFのブランド設定")}
        </h3>
        {onClose && (
      ...
      <p className="text-xs text-muted-foreground font-body">
        {text(
          "الشعار واللون المختاران يُطبَّقان تلقائيًا على جميع التقارير المطبوعة وملفات PDF وملفات الإكسل الملوّنة.",
          "Your logo and color are applied automatically to every printed report, PDF and colored Excel export.",
          "選択したロゴと色は、印刷レポート、PDF、カラー付きExcel出力のすべてに自動適用されます。"
        )}
      </p>

      {/* Logo */}
      <div className="space-y-2">
        <p className="text-xs font-medium font-body uppercase tracking-wider text-muted-foreground">{text("شعار الشركة", "Company logo", "会社ロゴ")}</p>
        <div className="flex items-center gap-3">
          ...
          >
            ...
            {text("رفع شعار", "Upload logo", "ロゴをアップロード")}
          </button>
          ...
          {logoUrl && (
            <button type="button" onClick={() => setLogoUrl("")} className="flex items-center gap-1 text-xs text-muted-foreground hover:text-destructive font-body">
              <Trash2 className="w-3.5 h-3.5" /> {text("إزالة", "Remove", "削除")}
            </button>
          )}
        </div>
      </div>
      ...
      {/* Color */}
      <div className="space-y-2">
        <p className="text-xs font-medium font-body uppercase tracking-wider text-muted-foreground">{text("لون الهوية", "Brand color", "ブランドカラー")}</p>
      ...
      <div className="space-y-2">
        <p className="text-xs font-medium font-body uppercase tracking-wider text-muted-foreground">{text("معاينة", "Preview", "プレビュー")}</p>
        <div className="rounded-lg border bg-white p-4" style={{ borderColor: color }}>
          <div className="flex items-center justify-between pb-3 mb-3" style={{ borderBottom: `3px solid ${color}` }}>
            <div>
              <p className="font-semibold text-sm text-gray-800">{text("تقرير المهام", "Tasks Report", "タスクレポート")}</p>
          ...
          <div className="flex gap-2">
            {[1, 2, 3].map((i) => (
              <div key={i} className="flex-1 rounded-md p-2" style={{ background: `${color}0d`, border: `1px solid ${color}33` }}>
                ...
                <p className="text-[9px] text-gray-500">{text("إحصائية", "Stat", "統計")}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      >
        ...
        {saved
          ? text("تم الحفظ ✓", "Saved ✓", "保存しました ✓")
          : text("حفظ وتطبيق على كل التقارير", "Save & apply to all reports", "保存してすべてのレポートに適用")}
