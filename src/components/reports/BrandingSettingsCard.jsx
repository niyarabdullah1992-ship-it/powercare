    ...
    <div className="rounded-xl border border-border bg-card p-5 space-y-5 max-w-2xl">
      <div className="flex items-center justify-between">
        <h3 className="font-heading text-lg font-semibold flex items-center gap-2">
          ...
          {ar ? "هوية التقارير وملفات PDF" : "Reports & PDF branding"}
        </h3>
        {onClose && (
      ...
      <div className="space-y-2">
        <p className="text-xs font-medium font-body uppercase tracking-wider text-muted-foreground">{ar ? "شعار الشركة" : "Company logo"}</p>
      ...
      {/* Color */}
      <div className="space-y-2">
        <p className="text-xs font-medium font-body uppercase tracking-wider text-muted-foreground">{ar ? "لون الهوية" : "Brand color"}</p>
      ...
      <div className="space-y-2">
        <p className="text-xs font-medium font-body uppercase tracking-wider text-muted-foreground">{ar ? "معاينة" : "Preview"}</p>
        <div className="rounded-lg border bg-white p-4" style={{ borderColor: color }}>
          <div className="flex items-center justify-between pb-3 mb-3" style={{ borderBottom: `3px solid ${color}` }}>
            <div>
              <p className="font-semibold text-sm text-gray-800">{ar ? "تقرير المهام" : "Tasks Report"}</p>
          ...
          <div className="flex gap-2">
            {[1, 2, 3].map((i) => (
              <div key={i} className="flex-1 rounded-md p-2" style={{ background: `${color}0d`, border: `1px solid ${color}33` }}>
                ...
                <p className="text-[9px] text-gray-500">{ar ? "إحصائية" : "Stat"}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      >
        ...
        {saved ? (ar ? "تم الحفظ ✓" : "Saved ✓") : ar ? "حفظ وتطبيق على كل التقارير" : "Save & apply to all reports"}
      </button>
    </div>
  );
}