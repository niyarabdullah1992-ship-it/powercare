...
export default function GroupPicker({ label, employees, selected, onToggle, accent = "bg-foreground", disabledIds = [] }) {
      ...
      {open && (
        <div className="absolute z-30 mt-1.5 w-full max-h-56 overflow-y-auto bg-card border border-border rounded-md shadow-xl py-1">
          {employees.map((e) => {
            ...
            const disabled = disabledIds.includes(e.id);
            return (
              <button
                key={e.id}
                type="button"
                ...
                className={`w-full flex items-center justify-between gap-2 px-2.5 py-1.5 text-xs font-body text-start ${disabled ? "opacity-40 cursor-not-allowed" : "hover:bg-muted"}`}
              >
