...
export default function PresentationVisual({ type, steps = [] }) {
  ...
  return <div className={`pc-device pc-${type}`}><div className="pc-device-bar"><span /><span /><span /></div><aside><div className="pc-mini-logo">P</div>{[1,2,3,4,5].map((n) => <i key={n} />)}</aside><main><div className="pc-visual-title"><Icon /><b>NiroVera</b></div><div className="pc-kpis"><span /><span /><span /></div><div className="pc-chart"><i /><i /><i /><i /><i /></div></main></div>;
}