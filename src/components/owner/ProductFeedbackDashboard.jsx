...
export default function ProductFeedbackDashboard({ ar, companies = [] }) {
  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-3 sm:grid-cols-3">{[[ar ? "إجمالي التقييمات" : "Total feedback", rows.length], [ar ? "متوسط التقييم" : "Average rating", average], [ar ? "النتائج المعروضة" : "Visible results", filtered.length]].map(([label, value]) => <div key={label} className="rounded-2xl bg-card p-5 shadow-sm"><p className="flex items-center gap-2 text-3xl font-bold">{value}{label.includes("متوسط") || label.includes("Average") ? <Star className="h-5 w-5 fill-landing-gold text-landing-gold" /> : null}</p><p className="text-xs text-muted-foreground">{label}</p></div>)}</div>
    ...
    <section className="rounded-2xl bg-card p-5 shadow-sm"><h2 className="flex items-center gap-2 font-heading text-xl font-semibold"><MessageSquare className="h-5 w-5 text-landing-gold" />{ar ? "التقييمات والملاحظات" : "Feedback & comments"}</h2><div className="mt-5 space-y-3">{filtered.map((row) => <article key={row.id} className="rounded-xl border border-border bg-background p-4"><div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground"><span className="font-bold text-landing-gold">{"★".repeat(row.rating)} {row.rating}/5</span><span>{names[row.companyId] || row.companyId}</span><span>·</span><span>{row.role || "—"}</span><span>·</span><span>{row.page || "—"}</span><span className="ms-auto">{new Date(row.created_date).toLocaleString(ar ? "ar-SA" : "en-GB")}</span></div><p className="mt-2 whitespace-pre-wrap text-sm leading-6">{row.message || "—"}</p></article>)}{!filtered.length && <p className="py-10 text-center text-sm text-muted-foreground">{ar ? "لا توجد نتائج." : "No matching feedback."}</p>}</div></section>
  </div>;
}