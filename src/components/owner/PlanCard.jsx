...
export default function PlanCard({ plan, ar, onEdit, onToggle, onDelete }) {
  ...
  return <article className="rounded-xl border border-border bg-card p-4 shadow-soft">
    ...
    <p className="mt-2 text-xs text-muted-foreground">{ar ? "المحطات" : "Stations"}: {plan.maxStations ?? (ar ? "غير محدود" : "Unlimited")} · {ar ? "الموظفون" : "Employees"}: {plan.maxEmployees ?? (ar ? "غير محدود" : "Unlimited")} · {(plan.enabledSections || []).length} {ar ? "قسم" : "sections"}</p>
