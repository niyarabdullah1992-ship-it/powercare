...
export default function PlanEntitlementsFields({ value, onChange, ar }) {
  ...
  return <div className="space-y-4 rounded-lg border border-border bg-muted/35 p-4 md:col-span-2">
    <div className="grid gap-3 sm:grid-cols-2">
      <label className="text-sm">{ar ? "الحد الأقصى للمحطات" : "Maximum stations"}<input type="number" min="1" value={value.maxStations ?? ""} onChange={(e) => limit("maxStations", e.target.value)} placeholder={ar ? "غير محدود" : "Unlimited"} className="mt-1 w-full rounded-md border border-input px-3 py-2" /></label>
