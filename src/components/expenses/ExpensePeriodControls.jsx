...
export default function ExpensePeriodControls({ period, setPeriod, days, setDays, from, setFrom, to, setTo, ar }) {
  return <div className="space-y-3">
    <div className="flex flex-wrap gap-2">
      {PERIODS.map(([value, arLabel, enLabel]) => <button key={value} type="button" onClick={() => setPeriod(value)} className={`rounded-full border px-3 py-1.5 text-xs font-body transition ${period === value ? "border-foreground bg-foreground text-background" : "border-border bg-card hover:bg-muted"}`}>{ar ? arLabel : enLabel}</button>)}
    </div>
