...
const LABELS = { travel: ["Travel", "سفر"], accommodation: ["Accommodation", "سكن"], fuel: ["Fuel", "وقود"], overtime_meals: ["Overtime Meals", "وجبات العمل الإضافي"], tools_equipment: ["Tools & Equipment", "أدوات ومعدات"], training: ["Training", "تدريب"], other: ["Other — write type", "أخرى — اكتب النوع"] };
...
export default function ExpenseForm({ stations, canPickStations, onSubmit, ar }) {
  ...
  const submit = async (event) => { event.preventDefault(); if (canPickStations && !count) return; setSaving(true); const form = event.currentTarget; const data = new FormData(form); const ok = await onSubmit({ expenseType: type, customExpenseType: data.get("customExpenseType"), amount, expenseDate: data.get("expenseDate"), description: data.get("description"), receipt: data.get("receipt"), stationScope: canPickStations ? scope : "single", stationIds: scope === "selected" ? selected : [] }); if (ok) { form.reset(); setType(""); setSelected([]); setAmount(0); } setSaving(false); };
  return <form onSubmit={submit} className="grid gap-3 rounded-2xl border border-border bg-card p-5 md:grid-cols-2 xl:grid-cols-5">
    <select required value={type} onChange={(event) => setType(event.target.value)} className="rounded-lg border px-3 py-2"><option value="">{ar ? "نوع المصروف" : "Expense type"}</option>{TYPES.map((item) => <option key={item} value={item}>{LABELS[item][ar ? 1 : 0]}</option>)}</select>
    {type === "other" && <input name="customExpenseType" required placeholder={ar ? "اكتب نوع المصروف" : "Write expense type"} className="rounded-lg border px-3 py-2" />}
