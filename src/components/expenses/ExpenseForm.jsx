...
export default function ExpenseForm({ stations, canPickStations, onSubmit, ar }) {
  const [saving, setSaving] = useState(false); const [type, setType] = useState(""); const [scope, setScope] = useState("all"); const [selected, setSelected] = useState([]); const [amount, setAmount] = useState(0); const [receipt, setReceipt] = useState(null);
  const count = canPickStations ? (scope === "all" ? stations.length : selected.length) : 1; const total = Number(amount || 0) * count;
  const submit = async (event) => { event.preventDefault(); if ((canPickStations && !count) || !receipt) return; setSaving(true); const form = event.currentTarget; const data = new FormData(form); const matchedType = TYPES.find((item) => LABELS[item][ar ? 1 : 0].toLowerCase() === type.trim().toLowerCase()); const ok = await onSubmit({ expenseType: matchedType || "other", customExpenseType: matchedType ? "" : type.trim(), amount, expenseDate: data.get("expenseDate"), description: data.get("description"), receipt, stationScope: canPickStations ? scope : "single", stationIds: scope === "selected" ? selected : [] }); if (ok) { form.reset(); setType(""); setSelected([]); setAmount(0); setReceipt(null); } setSaving(false); };
  return <form onSubmit={submit} className="grid gap-3 rounded-2xl border border-border bg-card p-5 md:grid-cols-2 xl:grid-cols-5">
    ...
    <label className="flex cursor-pointer items-center gap-2 rounded-lg border border-dashed border-accent/50 px-3 py-2 text-sm"><Upload className="h-4 w-4 shrink-0 text-accent" /><span className="min-w-0 truncate">{receipt?.name || (ar ? "رفع صورة الإيصال" : "Upload receipt image")}</span><input name="receipt" type="file" accept="image/*" required className="hidden" onChange={(event) => setReceipt(event.target.files?.[0] || null)} /></label>
    ...
    <input name="description" placeholder={ar ? "وصف مختصر" : "Short description"} className="rounded-lg border px-3 py-2 xl:col-span-4" /><button disabled={saving || !count || !receipt} className="flex items-center justify-center gap-2 rounded-lg bg-accent px-4 py-2 font-medium text-accent-foreground disabled:opacity-50">{saving && <Loader2 className="h-4 w-4 animate-spin" />}{ar ? "إرسال المصروف" : "Submit expense"}</button>
  </form>;
}