...
export default function SmartArchive({ targets, renderTask, t, lang, dir }) {
    ...
    <div className="space-y-4">
      ...
      {yearList.length === 0 ? (
        <div className="py-10 text-center space-y-2">
          ...
          <p className="text-sm text-muted-foreground font-body">{t("noArchivedTasks")}</p>
        </div>
      ) : (
        yearList.map((year) => (
          <div key={year} className="space-y-2">
            ...
            <div className="space-y-2">
              {groupTasksByPeriod(years.get(year), lang, t).map((grp) => (
                <ArchivePeriodGroup key={grp.key} group={grp} t={t} renderTask={renderTask} />
              ))}
            </div>
          </div>
        ))
