 ...
 */
export default function TaskStats({ targets, t, lang = "ar", counts = null }) {
  const ar = lang === "ar";
  // Prefer server-derived counters (operations.counts). Never invent literals when counts exist.
  if (counts) {
    const cards = [
      { label: ar ? "مهام نشطة" : "Active tasks", value: counts.active ?? 0 },
      { label: ar ? "بانتظار مراجعتك" : "Awaiting your review", value: counts.awaiting ?? 0 },
      { label: ar ? "متأخرة" : "Overdue", value: counts.overdue ?? 0 },
      { label: ar ? "اليوم" : "Due today", value: counts.today ?? 0 },
      { label: ar ? "نقاط ممنوحة" : "Points awarded", value: Math.round(counts.pointsAwarded ?? 0) },
    ];
    return (
      <div className="grid grid-cols-2 gap-3.5 sm:grid-cols-3 xl:grid-cols-5">
        {cards.map((c) => (
          <div key={c.label} className="flex flex-col gap-1.5 rounded-[10px] border border-[#E4E7EC] bg-white p-4">
  ...
  const awaiting = targets.filter((x) =>
    ["pending_review", "pending", "submitted", "awaiting_approval"].includes(x.status)
  ...
  const points = targets.reduce((sum, x) => sum + Number(x.points_awarded || x.pointsAwarded || x.points || 0), 0);
