...
export default function MultiSignPlacementModal({ docUrl, signers, initialSpots, ar, onConfirm, onClose }) {
  ...
  useEffect(() => { if (!pdfDoc || !canvasRef.current) return; let cancelled = false; (async () => { setLoading(true); const pdfPage = await pdfDoc.getPage(page); if (cancelled) return; const base = pdfPage.getViewport({ scale: 1 }); const host = wrapRef.current?.parentElement; const widthScale = Math.max(0.1, ((host?.clientWidth || 600) - 8) / base.width); const availableHeight = Math.max(300, window.innerHeight - 180); const heightScale = availableHeight / base.height; const cssScale = Math.min(widthScale, heightScale, 1.5); const ratio = window.devicePixelRatio || 1; const viewport = pdfPage.getViewport({ scale: cssScale * ratio }); const canvas = canvasRef.current; canvas.width = viewport.width; canvas.height = viewport.height; canvas.style.width = `${viewport.width / ratio}px`; canvas.style.height = `${viewport.height / ratio}px`; canvas.style.direction = "ltr"; const context = canvas.getContext("2d"); context.direction = "ltr"; await pdfPage.render({ canvasContext: context, viewport }).promise; if (!cancelled) setLoading(false); })(); return () => { cancelled = true; }; }, [pdfDoc, page]);
