...
export default function MultiSignPlacementModal({ docUrl, signers, initialSpots, signaturePreviews = [], activeSigner, onActiveSignerChange, ar, onConfirm, onChange, onClose, embedded = false }) {
  ...
  return <div className={embedded ? "w-full min-w-0" : "fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4"} onClick={embedded ? undefined : onClose}>
    <div className={embedded ? "flex min-h-[70vh] w-full flex-col overflow-hidden bg-card" : "flex max-h-[94vh] w-full max-w-5xl flex-col overflow-hidden rounded-3xl border border-accent/20 bg-card shadow-elevated"} onClick={(event) => event.stopPropagation()}>
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden bg-secondary/60 p-3"><div className="mx-auto mb-2 max-w-2xl shrink-0 rounded-xl border border-accent/20 bg-card px-4 py-2 text-center text-xs text-muted-foreground">{ar ? "يمكنك إضافة عدة توقيعات ونصوص لكل موقّع. حدّد أي حقل لتغيير حجمه أو حذفه." : "Add multiple signatures and text fields per signer. Select any field to resize or remove it."}</div><div ref={stageRef} className="flex min-h-0 flex-1 items-center justify-center overflow-hidden"><div ref={wrapRef} dir="ltr" onClick={placeField} className="relative cursor-crosshair bg-white shadow-md" style={{ touchAction: "none", direction: "ltr" }}><canvas ref={canvasRef} dir="ltr" className="block" />
