...
export default function MultiSignPlacementModal({ docUrl, signers, initialSpots, signaturePreviews = [], activeSigner, onActiveSignerChange, ar, onConfirm, onChange, onClose, embedded = false }) {
  ...
  return <div className={embedded ? "w-full min-w-0" : "fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4"} onClick={embedded ? undefined : onClose}>
    <div className={embedded ? "flex min-h-[78vh] w-full flex-col overflow-hidden bg-card" : "flex max-h-[94vh] w-full max-w-5xl flex-col overflow-hidden rounded-3xl border border-accent/20 bg-card shadow-elevated"} onClick={(event) => event.stopPropagation()}>
      ...
      <div className="relative flex min-h-0 flex-1 flex-col overflow-hidden bg-primary/95 p-3"><div className="pointer-events-none absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-transparent via-accent to-transparent" /><div className="mx-auto mb-2 max-w-2xl shrink-0 rounded-xl border border-accent/30 bg-card/95 px-4 py-2 text-center text-xs text-muted-foreground shadow-lg">{ar ? "يمكنك إضافة عدة توقيعات ونصوص لكل موقّع. حدّد أي حقل لتغيير حجمه أو حذفه." : "Add multiple signatures and text fields per signer. Select any field to resize or remove it."}</div><div ref={stageRef} className="flex min-h-0 flex-1 items-center justify-center overflow-auto p-1"><div ref={wrapRef} dir="ltr" onClick={placeField} className="relative cursor-crosshair bg-white shadow-2xl ring-1 ring-accent/35" style={{ touchAction: "none", direction: "ltr" }}><canvas ref={canvasRef} dir="ltr" className="block" />
