...
import { Inbox, Loader2, PenLine, Download, RefreshCw, Trash2, ShieldCheck, CalendarClock, BellRing, CheckCircle2 } from "lucide-react";
    ...
    <section className="space-y-4">
      ...
      {requests.length === 0 ? <div className="rounded-2xl border border-dashed border-accent/30 bg-card p-12 text-center text-sm text-muted-foreground">{ar ? "لا توجد طلبات توقيع حتى الآن." : "No signature requests yet."}</div> : <div className="grid gap-4">{requests.map((request) => {
        return <article key={request.id} className={`rounded-2xl border border-s-4 border-border bg-card p-5 shadow-sm transition-shadow hover:shadow-md sm:p-7 ${statusBorder(request.status)}`}>
          ...
          {request.isCreator && request.status === "pending" && <div className="mt-3 flex flex-wrap gap-2">{request.signers.filter((signer) => signer.status === "pending").map((signer) => { const key = `${request.id}:${signer.email}`; return <button key={signer.email} onClick={() => remind(request, signer)} disabled={remindingKey === key} className="inline-flex min-h-10 items-center gap-1.5 rounded-lg border border-accent/25 bg-accent/5 px-3 text-xs font-semibold text-accent hover:bg-accent/10 disabled:opacity-60">{remindingKey === key ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : remindedKey === key ? <CheckCircle2 className="h-3.5 w-3.5 text-emerald-600" /> : <BellRing className="h-3.5 w-3.5" />}{remindedKey === key ? (ar ? "تم التذكير" : "Reminder sent") : `${ar ? "تذكير" : "Remind"} ${signer.name}`}</button>; })}</div>}
