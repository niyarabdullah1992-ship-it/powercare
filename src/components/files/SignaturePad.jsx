...
export default function SignaturePad({ ar, signerName, verificationId, onPreview, onSave, saving }) {
  ...
  const end = async () => { drawing.current = false; previous.current = null; if (!inkRef.current) return; const composed = await makeSignatureStamp(canvasRef.current.toDataURL("image/png"), signerName, verificationId); setStamp(composed); onPreview?.(composed); };
  ...
  return <div className="space-y-4">
    ...
    <canvas ref={canvasRef} width={900} height={260} className="h-44 w-full touch-none cursor-crosshair rounded-xl border-2 border-dashed border-accent/35 bg-card shadow-inner" onPointerDown={start} onPointerMove={move} onPointerUp={end} onPointerCancel={end} onPointerLeave={end} />
    {stamp && <Image src={stamp} alt={ar ? "معاينة التوقيع" : "Signature preview"} fittingType="fit" className="mx-auto h-24 w-full max-w-sm" />}
    <div className="grid gap-2 sm:grid-cols-[auto_1fr]"><button type="button" onClick={clear} className="inline-flex items-center justify-center gap-2 rounded-lg border border-border px-4 py-3 text-sm font-medium hover:bg-secondary"><Eraser className="h-4 w-4" />{ar ? "مسح" : "Clear"}</button><button type="button" disabled={!stamp || saving} onClick={() => onSave(canvasRef.current.toDataURL("image/png"), signerName, "drawn")} className="inline-flex items-center justify-center gap-2 rounded-lg bg-primary px-6 py-3 text-sm font-bold text-primary-foreground disabled:opacity-40"><Check className="h-4 w-4" />{saving ? (ar ? "جارٍ الحفظ…" : "Saving…") : (ar ? "اعتماد التوقيع" : "Approve signature")}</button></div>
  </div>;
}