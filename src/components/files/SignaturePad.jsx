...
export default function SignaturePad({ ar, signerName, verificationId, onPreview, onSave, saving }) {
  ...
  return <div className="space-y-2.5">
    ...
    <div className="sticky bottom-0 z-10 grid gap-2 bg-card pt-1 sm:grid-cols-[auto_1fr]"><button type="button" onClick={clear} className="inline-flex items-center justify-center gap-2 rounded-lg border border-border px-4 py-2.5 text-sm font-medium hover:bg-secondary"><Eraser className="h-4 w-4" />{ar ? "مسح" : "Clear"}</button><button type="button" disabled={!stamp || saving} onClick={() => onSave(canvasRef.current.toDataURL("image/png"), signerName, "drawn")} className="inline-flex items-center justify-center gap-2 rounded-lg bg-primary px-6 py-2.5 text-sm font-bold text-primary-foreground disabled:opacity-40"><Check className="h-4 w-4" />{saving ? (ar ? "جارٍ الحفظ…" : "Saving…") : (ar ? "اعتماد التوقيع" : "Approve signature")}</button></div>
  </div>;
}