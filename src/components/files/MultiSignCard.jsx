...
export default function MultiSignCard({ currentUser, companyId, employees, ar, onCreated }) {
  ...
  const allPlaced = validSigners.length > 0 && validSigners.every((_, index) => (spots[index] || []).some((field) => field.type === "signature"));
  const labels = ar ? ["المستند", "الموقّعون", "وضع الحقول", "المراجعة والإرسال"] : ["Document", "Signers", "Place fields", "Review & send"];
  ...
  const next = () => {
    ...
    if (step === 3 && !allPlaced) return setError(ar ? "ضع حقل توقيع واحدًا على الأقل لكل موقّع." : "Place at least one signature field for every signer.");
