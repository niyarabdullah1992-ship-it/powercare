...
import { Upload, Loader2, FileText, Plus, X, Send, Copy, Check, MousePointerClick, Users, ChevronLeft, ChevronRight, CheckCircle2, UserRoundPen } from "lucide-react";
...
export default function MultiSignCard({ currentUser, companyId, employees, ar, onCreated }) {
  ...
  const addCurrentUser = () => {
    ...
    if (signers.some((signer) => signer.email.trim().toLowerCase() === email.toLowerCase())) {
      setError(ar ? "أنت موجود بالفعل في القائمة" : "You are already in the list");
      return;
    }
    setError("");
    setSigners((rows) => [...rows, { name: currentUser.name || "", email }]);
  };
