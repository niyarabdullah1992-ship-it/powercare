  ...
  return (
    <div className="grid items-start gap-4 md:grid-cols-[minmax(0,1fr)_280px]">
      <div className="min-w-0 space-y-4">
        <div className="rounded-xl border border-border bg-secondary/30 p-3"><label className="mb-1.5 block text-xs font-semibold text-foreground">{ar ? "1. اكتب اسم التوقيع" : "1. Enter signature name"}</label><input value={name} onChange={(event) => setName(event.target.value)} dir="auto" placeholder={ar ? "اكتب اسمك…" : "Type your name…"} className="h-11 w-full rounded-lg border border-input bg-card px-3 text-sm font-body outline-none focus:ring-2 focus:ring-ring" /></div>
        <div><div className="mb-2 flex items-center justify-between"><p className="text-xs font-semibold text-foreground">{ar ? "2. اختر نمط الكتابة" : "2. Choose a writing style"}</p><span className="text-[10px] text-muted-foreground">{selectedFont?.label}</span></div><div className="max-h-[310px] overflow-y-auto rounded-xl border border-border bg-secondary/20 p-2"><div className="grid grid-cols-2 gap-2">{FONTS.map((font) => <button key={font.id} type="button" onClick={() => setFontId(font.id)} disabled={!samples[font.id]} className={`relative min-h-20 rounded-lg border p-2 text-center text-foreground transition disabled:opacity-50 ${fontId === font.id ? "border-accent bg-accent/10 ring-1 ring-accent" : "border-border bg-card hover:border-accent/50"}`}>{samples[font.id] && <img src={samples[font.id]} alt={font.label} className="mx-auto h-9 w-full object-contain" />}<span className="mt-1 block text-[9px] text-muted-foreground">{font.label}</span>{fontId === font.id && <span className="absolute end-2 top-2 flex h-5 w-5 items-center justify-center rounded-full bg-accent text-accent-foreground"><Check className="h-3 w-3" /></span>}</button>)}</div></div></div>
      </div>
      <aside className="h-fit rounded-xl border border-accent/25 bg-secondary/30 p-3 md:sticky md:top-4"><p className="mb-3 text-xs font-semibold text-foreground">{ar ? "3. راجع واعتمد" : "3. Review and approve"}</p><div className="flex h-44 items-center justify-center rounded-lg border border-border bg-card p-3">{stamp ? <img src={stamp} alt={ar ? "معاينة الختم" : "Stamp preview"} className="max-h-full w-full object-contain" /> : <p className="text-center text-xs text-muted-foreground">{ar ? "ستظهر المعاينة هنا بعد اختيار النمط." : "Your preview will appear here."}</p>}</div><button type="button" disabled={!stamp || saving} onClick={save} className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-lg bg-primary px-5 py-3 text-sm font-semibold text-primary-foreground shadow-sm disabled:opacity-40"><Check className="h-4 w-4" />{saving ? (ar ? "جارٍ الحفظ…" : "Saving…") : ar ? "اعتماد التوقيع" : "Approve signature"}</button></aside>
    </div>
  );
}