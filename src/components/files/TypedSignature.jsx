  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        {FONTS.map((font) => <button key={font.id} type="button" onClick={() => setFontId(font.id)} disabled={!samples[font.id]} className={`rounded-2xl border px-3 py-3 text-center text-foreground transition disabled:opacity-50 ${fontId === font.id ? "border-accent bg-secondary ring-2 ring-accent/20" : "border-border bg-card hover:bg-secondary"}`}>{samples[font.id] && <Image src={samples[font.id]} alt={font.label} fittingType="fit" className="mx-auto h-12 w-full" />}<span className="mt-1 block text-[9px] text-muted-foreground">{font.label}</span></button>)}
      </div>
