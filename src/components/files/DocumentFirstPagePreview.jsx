...
export default function DocumentFirstPagePreview({ url, file, ar }) {
  ...
  return <div className="document-first-page-preview relative flex min-h-[70vh] w-full min-w-0 max-w-full items-start justify-center overflow-hidden bg-secondary/45 p-4"><canvas ref={canvasRef} className="document-first-page-canvas block h-auto min-w-0 w-full max-w-full bg-white shadow-lg" />{loading && <div className="absolute inset-0 flex items-center justify-center bg-card/70"><Loader2 className="h-6 w-6 animate-spin text-accent" /></div>}</div>;
}