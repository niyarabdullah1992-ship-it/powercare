...
export default function RandomSignaturePicker({ ar, signerName, verificationId, onSave, saving }) {
  ...
  return <div className="space-y-4">
    ...
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">{options.map((signature, index) => <button key={signature} type="button" onClick={() => setSelected(signature)} className={`relative aspect-[3/1] overflow-hidden rounded-xl border bg-card p-3 transition ${selected === signature ? "border-accent ring-2 ring-accent/30" : "border-border hover:border-accent/60"}`}><img src={signature} alt={`${ar ? "توقيع" : "Signature"} ${index + 1}`} className="h-full w-full object-contain" />{selected === signature && <span className="absolute end-2 top-2 rounded-full bg-accent p-1 text-accent-foreground"><Check className="h-3 w-3" /></span>}</button>)}</div>
