...
export default function EmployeeHierarchyTree({ sections, owner, company, t, ar, statusFor, onSelectStation, canReorder, onStationDragEnd }) {
  ...
  const changeZoom = (change) => setZoom(zoom + change);
  ...
  return <div className="rounded-2xl border border-accent/20 bg-muted/40">
    <div className="flex flex-wrap items-center justify-between gap-3 border-b border-accent/15 bg-card px-4 py-3"><div><p className="text-sm font-semibold">{ar ? "خريطة الموظفين والمحطات" : "Employees & stations map"}</p><p className="text-[11px] text-muted-foreground">{ar ? "اضغط على المحطة لإدارتها، واستخدم إصبعين للتكبير والتصغير" : "Select a station to manage it; pinch with two fingers to zoom"}</p></div><HierarchyZoomControls zoom={zoom} onZoom={changeZoom} onFit={() => setCustomZoom(null)} ar={ar} /></div>
    <div ref={viewportRef} {...gestures} className="overflow-auto p-4 pb-7 md:p-7" style={{ touchAction: "none" }}><div className="mx-auto min-w-max origin-top" style={{ zoom }}>
